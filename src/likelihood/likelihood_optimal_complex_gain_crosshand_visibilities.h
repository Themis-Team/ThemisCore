/*! 
  ...
  \date  March, 2020
  \brief Header file for the likelihood_optimal_complex_gain_crosshand_visibilities likelihood class
...
#ifndef THEMIS_LIKELIHOOD_OPTIMAL_COMPLEX_GAIN_CROSSHAND_VISIBILITIES_H_
#define THEMIS_LIKELIHOOD_OPTIMAL_COMPLEX_GAIN_CROSSHAND_VISIBILITIES_H_
...
namespace Themis
{
  ...
  /*! 
    ...
    assuming Gaussian priors on the \f$|G_A|\f$ and Gaussian errors.  Note that this likelihood is *not constrained*
    ...
    There are good reasons to believe that their relative calibration is much better than their absolute calibration. So this routine offers both options: Independent and Ratio.
  */
  class likelihood_optimal_complex_gain_crosshand_visibilities : public likelihood_base
  {
  public:

    enum class HandGainMode {
      Independent,   // free GR and GL
      Ratio          // common gain + R/L ratio
    };
    void set_hand_gain_mode(HandGainMode mode);
    void set_ratio_priors(double sigma_ratio_logamp, double sigma_ratio_phase);
    struct HandGains {
      std::complex<double> GR;
      std::complex<double> GL;
    };

    double wrap_phase_(double x) const;
    double sigma_ratio_logamp_eff_() const;
    double sigma_ratio_phase_eff_() const;
    
    void ratio_common_and_diff_(const HandGains& G,
				double& ac, double& pc,
				double& eta, double& delta) const;

    void encode_station_params_(const HandGains& G, double p4[]) const;
    HandGains decode_station_params_(const double g[], size_t s) const;
    
    HandGainMode _hand_gain_mode = HandGainMode::Independent;
    double _sigma_ratio_logamp = 0.0;
    double _sigma_ratio_phase  = 0.0;
    
    inline std::complex<double> baseline_gain_rr(const HandGains& g1, const HandGains& g2) const {
      return g1.GR * std::conj(g2.GR);
    }
    inline std::complex<double> baseline_gain_ll(const HandGains& g1, const HandGains& g2) const {
      return g1.GL * std::conj(g2.GL);
    }
    inline std::complex<double> baseline_gain_rl(const HandGains& g1, const HandGains& g2) const {
      return g1.GR * std::conj(g2.GL);
    }
    inline std::complex<double> baseline_gain_lr(const HandGains& g1, const HandGains& g2) const {
      return g1.GL * std::conj(g2.GR);
    }

    inline int idx_logamp_R(size_t s) const { return 4*int(s) + 1; }
    inline int idx_phase_R (size_t s) const { return 4*int(s) + 2; }
    inline int idx_logamp_L(size_t s) const { return 4*int(s) + 3; }
    inline int idx_phase_L (size_t s) const { return 4*int(s) + 4; }

    void require_independent_mode_only_(const char* where) const;
    
    //! Construct to do scan-by-scan correction by default
    likelihood_optimal_complex_gain_crosshand_visibilities(data_crosshand_visibilities& data, model_crosshand_visibilities& model, std::vector<std::string> station_codes, std::vector<double> sigma_g);

    //! Specify the times of the gain-correction epochs by hand
    likelihood_optimal_complex_gain_crosshand_visibilities(data_crosshand_visibilities& data, model_crosshand_visibilities& model, std::vector<std::string> station_codes, std::vector<double> sigma_g, std::vector<double> t_ge);

    //! Specify the times of the gain-correction epochs by hand
    likelihood_optimal_complex_gain_crosshand_visibilities(data_crosshand_visibilities& data, model_crosshand_visibilities& model, std::vector<std::string> station_codes, std::vector<double> sigma_g, std::vector<double> t_ge, std::vector<double> max_g);

    //! Construct to do scan-by-scan correction by default
    likelihood_optimal_complex_gain_crosshand_visibilities(data_crosshand_visibilities& data, model_crosshand_visibilities& model, uncertainty_crosshand_visibilities& visibility, std::vector<std::string> station_codes, std::vector<double> sigma_g);

    //! Specify the times of the gain-correction epochs by hand
    likelihood_optimal_complex_gain_crosshand_visibilities(data_crosshand_visibilities& data, model_crosshand_visibilities& model, uncertainty_crosshand_visibilities& visibility, std::vector<std::string> station_codes, std::vector<double> sigma_g, std::vector<double> t_ge);

    //! Specify the times of the gain-correction epochs by hand
    likelihood_optimal_complex_gain_crosshand_visibilities(data_crosshand_visibilities& data, model_crosshand_visibilities& model, uncertainty_crosshand_visibilities& visibility, std::vector<std::string> station_codes, std::vector<double> sigma_g, std::vector<double> t_ge, std::vector<double> max_g);
 ...
 // std::vector< std::vector< std::complex<double> > >  get_gains();
    std::vector< std::vector< HandGains > >  get_gains();
  ...
  // std::vector< std::vector< std::complex<double> > > _G;
     std::vector< std::vector< HandGains > > _G;
    ...
    double optimal_complex_gains(std::vector< std::vector< std::complex<double> > >& y, std::vector< std::vector< std::complex<double> > >& yb, std::vector<size_t>& is1, std::vector<size_t>& is2, std::vector< HandGains >& gest);

double optimal_complex_gains_trial(std::vector< std::vector< std::complex<double> > >& y, std::vector< std::vector< std::complex<double> > >& yb, std::vector<size_t>& is1, std::vector<size_t>& is2, std::vector< HandGains >& gest, double& chisq);

double optimal_complex_gains_log_trial(std::vector< std::vector< std::complex<double> > >& y, std::vector< std::vector< std::complex<double> > >& yb, std::vector<size_t>& is1, std::vector<size_t>& is2, std::vector< HandGains >& gest, double& chisq_opt);
