/*!
  ...
  \author Paul Tiede
  \brief Implementation file for the sampler_stan_adapt_diag_e_nuts_MCMC.
*/
...
sampler_stan_adapt_diag_e_nuts_MCMC::sampler_stan_adapt_diag_e_nuts_MCMC(int seed, likelihood& L, std::vector<std::string> var_names, size_t dimension)
      ...
      _restart(false)
{
  ...
  _term_buffer = 50;
  _window = 25;
  _inverse_metric.resize(_dimension, 1.0);
  Eigen::VectorXd inv_metric(_dimension);
  for ( size_t i = 0; i < _dimension; ++i )
    inv_metric(i) = _inverse_metric[i];
  stan::services::util::validate_diag_inv_metric(inv_metric,_logger);
  _stan_sampler.set_metric(inv_metric);
  
}//end sampler_stan_adapt_diag_e_nuts_MCMC


void sampler_stan_adapt_diag_e_nuts_MCMC::set_initial_inverse_metric(std::vector<double> inverse_metric)
{
  _inverse_metric = inverse_metric;
  if (inverse_metric.size() != _dimension){
    std::cerr << "Error: set_initial_inverse_metric::Inverse metric must have same dimension as problem\n";
    std::exit(1);
  }
  Eigen::VectorXd inv_metric(_dimension);

  for ( size_t i = 0; i < _dimension; ++i )
    inv_metric(i) = inverse_metric[i];
  stan::services::util::validate_diag_inv_metric(inv_metric,_logger);
  _stan_sampler.set_metric(inv_metric); 
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_window_parameters(unsigned int init_buffer, unsigned int term_buffer, unsigned int window)
{
  _init_buffer = init_buffer;
  _term_buffer = term_buffer;
  _window = window;
}
void sampler_stan_adapt_diag_e_nuts_MCMC::set_adaptation_parameters(int nadapts, bool save_adaptation)
{
  _nadapt = nadapts;
...
void sampler_stan_adapt_diag_e_nuts_MCMC::set_initial_location(std::vector<double> initial_parameters)
{
...
void sampler_stan_adapt_diag_e_nuts_MCMC::reset_rng_seed(int seed)
{
  _seed = seed;
  _stan_rng.seed(_seed);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_stepsize(double step_size)
{
  _stan_sampler.set_nominal_stepsize(step_size);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_delta(double delta)
{
  _stan_sampler.get_stepsize_adaptation().set_delta(delta);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_gamma(double gamma)
{
  _stan_sampler.get_stepsize_adaptation().set_gamma(gamma);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_kappa(double kappa)
{
  _stan_sampler.get_stepsize_adaptation().set_kappa(kappa);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_t0(double t0)
{
  _stan_sampler.get_stepsize_adaptation().set_t0(t0);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::set_max_depth(int max_depth)
{
  _stan_sampler.set_max_depth(max_depth);
}

void sampler_stan_adapt_diag_e_nuts_MCMC::run_sampler(int nsteps, 
                                                      int thin, int refresh,
                                                      int verbosity)
{
  //Open the chains for output
  ...
  //Now lets see if we should adapt
  if ((_step_count == 0 && _nadapt > 0) || 
      (_restart && _nadapt > 0 && _step_count < (size_t)_nadapt)){
    _adaptation = true;
...
void sampler_stan_adapt_diag_e_nuts_MCMC::generate_transition( int step, int start, int finish,
...
void sampler_stan_adapt_diag_e_nuts_MCMC::write_checkpoint(std::ostream& out)
{
  ...
  Eigen::VectorXd metric = _stan_sampler.z().inv_e_metric_;
  for ( int i = 0; i < metric.size(); ++i )
    out << std::setw(25) << metric(i);
  out << std::endl;
...
void sampler_stan_adapt_diag_e_nuts_MCMC::read_checkpoint(std::istream& ckpt_in)
{
  
  MPI_Comm_rank(_comm, &_rank);
  std::vector<double> inv_metric(_dimension), chain(_dimension), state;
  ...
  //MPI_Bcast(&inv_metric[0], dimension, MPI_DOUBLE, 0, _comm);
 ...
 // }
  if (step_count < nadapt){
    if ( _rank == 0 ){
    std::cout << "sampler_stan_adapt_diag_e_nuts_MCMC::read_checkpoint Adaptation phase didn't finish last time!\n"
  ...
  //MPI_Barrier(_comm);
  this->set_initial_inverse_metric(inv_metric);
...
void sampler_stan_adapt_diag_e_nuts_MCMC::get_chain_state(std::vector<double>& model_values, std::vector<double>& state_values)
{
...
void sampler_stan_adapt_diag_e_nuts_MCMC::set_chain_state(std::vector<double> model_values, std::vector<double> state_values)
{
  double log_jacobian = 0.0;
  //change the model_values to live in the continuous space.
...
void sampler_stan_adapt_diag_e_nuts_MCMC::set_mpi_communicator(MPI_Comm comm)
{
  _comm = comm;
  sampler_MCMC_base::set_mpi_communicator(_comm);
  _L->set_mpi_communicator(_comm);
  _stan_model.set_mpi_communicator(_comm);
}


void sampler_stan_adapt_diag_e_nuts_MCMC::write_state()
{
  std::vector<double> model_values, state_values;
  get_chain_state(model_values, state_values);
...
void sampler_stan_adapt_diag_e_nuts_MCMC::write_chain_header()
{
...
void sampler_stan_adapt_diag_e_nuts_MCMC::write_state_header()
{
...
void sampler_stan_adapt_diag_e_nuts_MCMC::write_sampler_header()
{
  int id;
  ...
  if (id == 0){
    std::stringstream ss;
    for ( size_t i = 0; i < _dimension; ++i )
      ss << std::setw(15) << _inverse_metric[i];
    _sampler_out << "sampler = diagstan\n";
