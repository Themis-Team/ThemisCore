/*!
  \file optimizer_powell.cpp
  \author Avery Broderick
  \brief Implementation file for the optimizer_powell class
*/
...
namespace Themis{
  ...
  optimizer_powell::optimizer_powell(int seed)
    : _num_likelihood(1), _rng(seed)
  {
  }

  optimizer_powell::~optimizer_powell()
  {
  }

  void optimizer_powell::set_cpu_distribution(int num_likelihood)
  {
    _num_likelihood = num_likelihood;
  }


  std::vector<double> optimizer_powell::run_optimizer(likelihood& L, std::vector< std::vector<double> > start_parameter_values, std::string optimizer_results_filename, size_t number_of_restarts, double tolerance)
  {
    ...
    // Create a set of communicators for the likelihoods and powelles
    ...
    std::cout << "rank=" << rank << " L_rank=" << L_rank << " W_rank=" << W_rank << std::endl;
	...
	for (size_t r=0; r<number_of_restarts; ++r)
	  get_optimal_point(start_parameter_values[W_rank],tolerance);

	// Save the likelihood of the best
	Lbest_list.push_back( get_optimal_point(start_parameter_values[i],tolerance) );
  ...
  std::vector<double> optimizer_powell::run_optimizer(likelihood& L, std::vector<double> start_parameter_values, std::string optimizer_results_filename, size_t number_of_instances, size_t number_of_restarts, double tolerance)
  {
    // Get the number of parameters for the model
    _ndim = L.priors().size();
    ...
    // Run the optimizer
    return ( run_optimizer(L,start_plist,optimizer_results_filename,number_of_restarts,tolerance) );
  }

  std::vector<double> optimizer_powell::run_optimizer(likelihood& L, std::string optimizer_results_filename, size_t number_of_instances, size_t number_of_restarts, double tolerance)
  {
    // Get the number of parameters for the model
    _ndim = L.priors().size();
  ...
  std::vector<double> optimizer_powell::generate_start_point(likelihood& L)
  {
  ...
  double optimizer_powell::get_optimal_point(std::vector<double>& pstart, double tolerance)
  {  
    ...
    powell(p,xi,tolerance,iter,fret);


    std::cerr << "CONGRATS! optimizer_powell.get_optimal_point finished in " << iter << " evaluations\n";
  ...
  double optimizer_powell::func(double p[])
  {
    std::vector<double> pvec(_ndim);
    for (size_t j=0; j<_ndim; ++j)
      pvec[j] = p[j+1];
    return ( -_Lptr->operator()(pvec) );
  }

  double optimizer_powell::f1dim(double x)
  {
    int j;
    double f;
...
#define ITMAX 200
  ...
  void optimizer_powell::powell(double p[], double **xi, double ftol, int& iter, double& fret)
  {
    int n = int(_ndim);
	...
	return;
      ...
      if (iter == ITMAX) {
	std::cerr << "WARNING! In optimizer_powell::powell ITMAX=" << ITMAX << " exceeded\n";
	delete[] pt;
	delete[] ptt;
	delete[] xit;
	delete[] _pcom;
...
#define TOL 2.0e-4
  void optimizer_powell::linmin(double p[], double xi[], double& fret)
  {
    int n = int(_ndim);
    int j;
...
#define SHFT(a,b,c,d) (a)=(b);(b)=(c);(c)=(d);
  ...
  double optimizer_powell::brent(double ax, double bx, double cx, double tol, double& xmin)
  {
    int iter;
    ...
    std::cerr << "WARNING! In optimizer_powell::brent too many interations\n";
    xmin=x;
    return fx;
  }
#undef ITMAX
...
#define SHFT(a,b,c,d) (a)=(b);(b)=(c);(c)=(d);
  ...
  void optimizer_powell::mnbrak(double *ax, double *bx, double *cx, double *fa, double *fb, double *fc)
  {
