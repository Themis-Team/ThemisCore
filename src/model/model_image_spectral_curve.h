/*!
  \file model_image_spectral_curve.h
  \author Paul Tiede
  \date  Dec, 2020
  \brief Header file for the model_image_spectral_curve class, which generates a multiwavelength model by adding parameters that depend on the spectral curvature of some order 
  \details To be added
*/

#ifndef Themis_MODEL_IMAGE_SPECTRAL_CURVE_H_
#define Themis_MODEL_IMAGE_SPECTRAL_CURVE_H_
...
*/
class model_image_spectral_curve : public model_image
{
 private:
 ...
 public:
  ...
  model_image_spectral_curve(model_image& image, std::vector<int> order, double freqref);

  //! Constructor.  Takes a model_image object, single order, and optional reference time.
  model_image_spectral_curve(model_image& image, int order, double freqref);

  //! Constructor.  Takes a pair of model_image objects.
  //model_image_spectral_curve(model_image& image1, model_image& image2);
  virtual ~model_image_spectral_curve() {};

  //! Access to set reference time after construction.
  void set_reference_freq(double freq);

  //! Access to access reference time.
  double reference_freq() const;
  ...
  void generate_image_model(double freq);
