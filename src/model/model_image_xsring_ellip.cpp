/*!
  \file model_image_xsring_ellip.cpp
  \author Jorge Alejandro Preciado-Lopez
  \date  November 2018
  \brief Implements the geometric eight-parameter slashed elliptical ring (xsring) model image class.
  \details To be added
*/

#include "model_image_xsring_ellip.h"
#include <iostream>
#include <iomanip>
#include <complex>
#include <mpi.h>
...
namespace Themis {
  ...
  model_image_xsring_ellip::model_image_xsring_ellip()
    : _Nray(128), _use_analytical_visibilities(true)
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "Creating model_image_xsring_ellip in rank " << world_rank << std::endl;
  }

  void model_image_xsring_ellip::use_numerical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_xsring_ellip::use_analytical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_xsring_ellip::set_image_resolution(int Nray)
  {
    _Nray = Nray;
    
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "model_image_xsring_ellip: Rank " << world_rank << " using image resolution " << _Nray << std::endl;
  }
  

  void model_image_xsring_ellip::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
  {
    int Ntot = _Nray;
    ...
    */
    ...
    _tau = std::max(parameters[4],1e-4);
    _phi  = parameters[5];
    _f = std::min(std::max(parameters[6],1e-4),0.9999);
	...
	if (alpha[j].size()!=beta[j].size() || beta[j].size()!=I[j].size() || I[j].size()!=size_t(Ntot))
	{
      ...
      double scale = 1/(1-_tau);
      double alpha_min = -Nr*_Rext/scale;
      double alpha_max = Nr*_Rext/scale;
      double beta_min = -Nr*_Rext/scale;
      double beta_max = Nr*_Rext/scale;
	...
	for (size_t k=0; k<alpha[j].size(); ++k)
	  ...
	  I[j][k] = 0.0;
          double alphap = (std::cos(_phi)*alpha[j][k] + std::sin(_phi)*beta[j][k])*scale;
          double betap = (-std::sin(_phi)*alpha[j][k] + std::cos(_phi)*beta[j][k])/scale;
	  I[j][k] = _standard_image(alphap, betap);
...
double model_image_xsring_ellip::_standard_image(double alpha, double beta)
{

    double Iring_norm = (2.*_V0/M_PI) * 1./((1. + _f)*(_Rext*_Rext - _Rint*_Rint) - (1 - _f)*_d*_Rint*_Rint/_Rext);
    double I = 0;
    if (alpha*alpha + beta*beta <= _Rext*_Rext) {
	    //I[j][k] = 0.5 * (1 + alpha[j][k]/_Rext) + 0.5 * _f * (1 - alpha[j][k]/_Rext);
	  I = 0.5 * (1 - alpha/_Rext) + 0.5 * _f * (1 + alpha/_Rext);
    }
	  
    if ( ((alpha-_d)*(alpha-_d) + (beta)*(beta)) <= _Rint*_Rint) {
      I = 0.0;
    }
...
std::complex<double> model_image_xsring_ellip::_standard_vis(double u, double v)
{
  ...
  std::complex<double> exponent, Vring, V;
  exponent = -2.*M_PI*i*_d*u ;
      

  Vring = (M_PI*H/k) * (1 + _f) * _Rext*BesselJ1(k*_Rext)
      - (M_PI*H/k) * ((1 + _f) - (1 - _f)*_d/_Rext) * std::exp(exponent)*_Rint*BesselJ1(k*_Rint)
      - (i*M_PI*H/(2.*k*k)) * 2.*M_PI*u * (1 - _f) * ( _Rext*BesselJ0(k*_Rext) - _Rext*BesselJ2(k*_Rext) - 2.*BesselJ1(k*_Rext)/k )
      + (i*M_PI*H/(2.*k*k)) * 2.*M_PI*u * (1 - _f) * ( _Rint*BesselJ0(k*_Rint) - _Rint*BesselJ2(k*_Rint) - 2.*BesselJ1(k*_Rint)/k ) * (_Rint/_Rext) * std::exp(exponent);
  ...
  double model_image_xsring_ellip::BesselJ0(double x)
  {
    double ax,z;
  ...
  double model_image_xsring_ellip::BesselJ1(double x)
  {
    double ax,z;
    double xx,y,ans,ans1,ans2;
  ...
  double model_image_xsring_ellip::BesselJ2(double x)
  {
    return (2*BesselJ1(x)/x - BesselJ0(x));
  }
  
  
  std::complex<double> model_image_xsring_ellip::complex_visibility(double u, double v)
    {

      double scale = 1.0/(1.0-_tau);
      double rru = (std::cos(_phi)*u - std::sin(_phi)*v)/scale;
      double rrv = (std::sin(_phi)*u + std::cos(_phi)*v)*scale;
      
      double ru = rru*std::cos(_position_angle) + rrv*std::sin(_position_angle);
      double rv = -rru*std::sin(_position_angle) + rrv*std::cos(_position_angle);
      ...
      */
      ...
      return _standard_vis(ru, rv);
    }
    
  
  double model_image_xsring_ellip::visibility_amplitude(datum_visibility_amplitude& d, double acc)
  {
  ...
  double model_image_xsring_ellip::closure_phase(datum_closure_phase& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  double model_image_xsring_ellip::closure_amplitude(datum_closure_amplitude& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  std::complex<double> model_image_xsring_ellip::visibility(datum_visibility& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
