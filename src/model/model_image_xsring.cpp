...
namespace Themis {
  ...
  model_image_xsring::model_image_xsring()
    : _Nray(128), _use_analytical_visibilities(true)
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "Creating model_image_xsring in rank " << world_rank << std::endl;
  }

  void model_image_xsring::use_numerical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_xsring::use_analytical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_xsring::set_image_resolution(int Nray)
  {
    _Nray = Nray;
    
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "model_image_xsring: Rank " << world_rank << " using image resolution " << _Nray << std::endl;
  }
  

  void model_image_xsring::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
  {
    int Ntot = _Nray;
    ...
    //double Inorm = (2.*_V0/M_PI) * 1./((1. + _f)*(_Rext*_Rext - _Rint*_Rint));
  ...
  double model_image_xsring::BesselJ0(double x)
  {
    double ax,z;
  ...
  double model_image_xsring::BesselJ1(double x)
  {
    double ax,z;
    double xx,y,ans,ans1,ans2;
  ...
  double model_image_xsring::BesselJ2(double x)
  {
    return (2*BesselJ1(x)/x - BesselJ0(x));
  }
  
  
  std::complex<double> model_image_xsring::complex_visibility(double u, double v)
    {
      ...
      double H = (2.*_V0/M_PI) * 1./((1. + _f)*(_Rext*_Rext - _Rint*_Rint) - (1 - _f)*_d*_Rint*_Rint/_Rext);
      ...
      V = (M_PI*H/k) * (1 + _f) * _Rext*BesselJ1(k*_Rext)
  ...
  double model_image_xsring::visibility_amplitude(datum_visibility_amplitude& d, double acc)
  {
  ...
  double model_image_xsring::closure_phase(datum_closure_phase& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  double model_image_xsring::closure_amplitude(datum_closure_amplitude& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  std::complex<double> model_image_xsring::visibility(datum_visibility& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
