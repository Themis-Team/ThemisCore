...
model_image_shearing_spot::model_image_shearing_spot(double start_obs, double tobs, std::string sed_fit_parameter_file, double frequency, double M, double D)
...
void model_image_shearing_spot::open_error_streams()
{
#if 0
  if (_merr.is_open())
    _merr.close();
...
std::string model_image_shearing_spot::model_tag() const
{
  std::stringstream tag;
  tag << "model_image_shearing_spot " << _start_obs << " " << _tobs << " " << _frequency << " " << _M << " " << _D;
    return tag.str();
}

void model_image_shearing_spot::set_image_resolution(int Nray)
{
  _Nray=Nray;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_shearing_spot: Rank " << rank << " using image resolution " << _Nray << std::endl;
}

void model_image_shearing_spot::set_screen_size(double Rmax)
{
	_Rmax = Rmax;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_shearing_spot: Rank " << rank << " using screen size " << _Rmax << std::endl;

}

void model_image_shearing_spot::add_background_riaf()
{
  _bkgd_riaf=true;
  
  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_shearing_spot: Rank " << rank << " added backgroud sed_fitted_riaf " << std::endl;
}


void model_image_shearing_spot::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  int myrank;
  MPI_Comm_rank(_comm,&myrank);
  Themis::StopWatch sw;
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_shearing_spot::generate_image started at: ";
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_shearing_spot::generate_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
