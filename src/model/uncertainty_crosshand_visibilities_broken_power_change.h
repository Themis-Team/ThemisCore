/*!
  \file uncertainty_crosshand_visibilities_broken_power_change.h
  \author Avery E. Broderick
  \date  March, 2022
  \brief Header file for uncertainty model class approprirate for crosshand visibilities that adds a floor and fractional component.
  \details To be added
*/

#ifndef Themis_UNCERTAINTY_CROSSHAND_VISIBILITIES_BROKEN_POWER_CHANGE_H_
#define Themis_UNCERTAINTY_CROSSHAND_VISIBILITIES_BROKEN_POWER_CHANGE_H_
...
/*! 
  ...
  \details The uncertainty is \f$ \sigma_{RR,LL,RL,LR} = \sqrt{ \Re(\sigma_V)^2 + t^2 + f^2 |V|^2 + a^2/[1+(|u|/u_0)^b] } + i \sqrt{ \Im(\sigma_V)^2 + t^2 + f^2 |V| + a^2/[1+(|u|/u_0)^b] } \f$, corresponding to the "broken_power change" model.

  Parameter list:\n
    - parameters[0] ... Threshold to add in Jy, \f$t\f$.
    - parameters[1] ... Fraction of Stokes I visibility amplitude to add, \f$f\f$.
  ...
  \warning 
*/
class uncertainty_crosshand_visibilities_broken_power_change : public uncertainty_crosshand_visibilities
{
 public:
  uncertainty_crosshand_visibilities_broken_power_change();
  virtual ~uncertainty_crosshand_visibilities_broken_power_change();

  //! A user-supplied one-time generate function that permits uncertainty model construction prior to calling the crosshand visibilities for each datum.  Takes a vector of parameters.
  ...
  //! Returns the complex crosshand visibilities error in Jy computed given a datum_crosshand_visibilities object, containing all of the accoutrements.
  ...
  //! Specify the ratio between the uncertainties on the parallel and crosshands
  void set_crosshand_ratio(double ratio);
