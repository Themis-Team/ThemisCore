/*!
  ...
  \date  November 2018
  \brief Implements a Ring Model Image class.
  \details To be added
*/
...
namespace Themis {
  ...
  model_image_ring::model_image_ring()
    : _Nray(128), _use_analytical_visibilities(true)
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "Creating model_image_ring in rank " << world_rank << std::endl;
  }

  void model_image_ring::use_numerical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_ring::use_analytical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_ring::set_image_resolution(int Nray)
  {
    _Nray = Nray;
    
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "model_image_ring: Rank " << world_rank << " using image resolution " << _Nray << std::endl;
  }
  

  void model_image_ring::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
  {
    int Ntot = _Nray;
    ...
    for (size_t j=0; j<alpha.size(); ++j)
    {
      for (size_t k=0; k<alpha[j].size(); ++k)
      {
        ...
        if (alpha[j][k]*alpha[j][k] + beta[j][k]*beta[j][k] <= _Rext*_Rext) { I[j][k] = 1.0; }        
        if (alpha[j][k]*alpha[j][k] + beta[j][k]*beta[j][k] <= _Rint*_Rint) { I[j][k] = 0.0; }
  ...
  double model_image_ring::BesselJ1(double x)
  {
    double ax,z;
  ...
  std::complex<double> model_image_ring::complex_visibility(double u, double v)
    {
  ...
  double model_image_ring::visibility_amplitude(datum_visibility_amplitude& d, double acc)
  {
  ...
  double model_image_ring::closure_phase(datum_closure_phase& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  double model_image_ring::closure_amplitude(datum_closure_amplitude& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  std::complex<double> model_image_ring::visibility(datum_visibility& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
