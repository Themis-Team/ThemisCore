/*!
  \file model_image_general_riaf_waxion.cpp
...
model_image_general_riaf::model_image_general_riaf(double frequency, double D)
  : _comm(MPI_COMM_WORLD), _D(D), _frequency(frequency), _Rmax(15.0), _Nray_base(128), _Nray(128), _number_of_refines(0)
{
  open_error_streams();
}

void model_image_general_riaf::open_error_streams()
{
#if 0
  if (_merr.is_open())
    _merr.close();
...
void model_image_general_riaf::set_screen_size(double imgR)
{
  _Rmax=imgR;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_general_riaf: Rank " << rank << " using screen size " << _Rmax << std::endl;

}
void model_image_general_riaf::set_image_resolution(int Nray, int number_of_refines)
{
  _Nray_base=Nray;
  _Nray = _Nray_base;
...
std::string model_image_general_riaf::model_tag() const
{
  std::stringstream tag;
  tag << "model_image_general_riaf " << _frequency << " " << _D;
  return tag.str();
}
  
void model_image_general_riaf::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  int myrank;
  ...
  VRT2::RT_Axion rt_ax(g,avf_zamo,parameters[0],parameters[13],parameters[14]); // M in Msun, ma  in eV, ga (dimensionless)
  rts.push_back(&rt_ax);
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_general_riaf::generate_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
...
double model_image_general_riaf::generate_flux_estimate(double accuracy, std::vector<double> parameters)
{
  /////////////////////////////////////////////////////////
  ...
  //std::cerr << "model_image_general_riaf::generate_flux_estimate : generated first image " << I_new << "\n";
  ...
  do                                  //now we keep refining until certain conditions are met
  {
    ...
    /*std::cerr << "model_image_general_riaf::generate_flux_estimate : " << refinement_level
	      << std::setw(15) << I_old 
	      << std::setw(15) << I_new << "\n";*/
...
double model_image_general_riaf::estimate_image_size_log_spiral(VRT2::PolarizationMap &pmap, double initial_r, double theta_step, 
							double percent_from_max_I, unsigned int max_iter, double safety_factor)
{
  ...
  while (i < max_iter && !found_size)
  {
    ...
    /*
    std::cerr << "model_image_general_riaf::estimate_image_size_log_spiral : "
