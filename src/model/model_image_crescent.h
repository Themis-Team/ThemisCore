/*!
  ...
  \date  June, 2017
  \brief Header file for the Geometric Crescent Model Image class.
  \details To be added.
*/

#ifndef THEMIS_MODEL_IMAGE_CRESCENT_H_
#define THEMIS_MODEL_IMAGE_CRESCENT_H_
...
namespace Themis {
  ...
  /*!
    \brief Defines an interface for a Geometric Crescent Model based on
      the model_image class. This is a simple example of a model_image object.

    \details The Geometric Crescent Model is created by substracting out a
    disc from the inside of a larger disc, both of constant intensity.

    This model is defined by five parameters:
      ...
      - parameters[3]: \f$ \tau \f$ = The degree of symmetry
      - parameters[4]: \f$ \phi \f$ = The orientation (in model_image) in radians
  */
  class model_image_crescent : public model_image
  {
    private:
      
      /*! \brief Generates and returns rectalinear grid of intensities 
      associated with the Crescent Image model in Jy/str located 
    ...
    public:
      ...
      model_image_crescent();
      virtual ~model_image_crescent() {};
      ...
      {
	return "model_image_crescent";
      };
      
    private:
      double _Itotal;  //!< Internal total intensity.
      double _Rp;      //!< Radius of the larger disc.
      double _Rn;      //!< Radius of the smaller disc.
      double _a;       //!< <em>x</em>-coordinate of the centroid in uas.
      double _b;       //!< <em>y</em>-coordinate of the centroid in uas.
...
#endif  // THEMIS_MODEL_IMAGE_CRESCENT_H_
