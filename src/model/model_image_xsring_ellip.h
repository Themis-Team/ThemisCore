/*!
	\file model_image_xsring_ellip.h
	\author Jorge Alejandro Preciado-Lopez, Paul Tiede
	\date  Dec 2020
	\brief Header file for the geometric eight-parameter slashed elliptical ring (xsring) model image class.
	\details
*/

#ifndef THEMIS_MODEL_IMAGE_XSRINGAUSS_ELLIP_H_
#define THEMIS_MODEL_IMAGE_XSRINGAUSS_ELLIP_H_
...
namespace Themis {
  ...
  /*!
    ...
    \details The Geometric Slashed Elliptical Ring Model is created by substracting out a
    ...
    - parameters[1]: \f$R \f$ = The geometric mean of semi-major and semi-minor.
    - parameters[2]: \f$ \psi \f$ = The relative thickness.
    - parameters[3]: \f$ \fw \f$ = The inner offset.
    - parameters[4]: \f$ \tau \f$ = The ring asymmetry which is 1-semi-minor/semi-major .
    - parameters[5]: \f$ \phi \f$ = The ellipticity orientation (in model_image) in radians.
    - parameters[6]: \f$ f \f$ = The fading parameter.
    - parameters[7]: \f$ \xi \f$ = The brightness orientation (in model_image) in radians.
  */
  class model_image_xsring_ellip : public model_image
  {
   private:
  ...
  public:
    ...
    model_image_xsring_ellip();
    virtual ~model_image_xsring_ellip() {};
    ...
    {
      return "model_image_xsring_ellip";
    };
  ...
  private:
    ...
    double _tau; //!< Ring asymmetry
    double _phi; //!< Ring asymmetry orientation
    ...
    double _standard_image(double alpha, double beta);
    std::complex<double> _standard_vis(double u, double v);
