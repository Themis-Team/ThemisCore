...
model_visibility_galactic_center_diffractive_scattering_screen::model_visibility_galactic_center_diffractive_scattering_screen(
   ...
   _P_phi_func("none", scattering_model, scatt_alpha, _phi0)
{
  ...
  std::cerr << "Scattering your visibility model\n";
...
double model_visibility_galactic_center_diffractive_scattering_screen::Dphi(double u, double v) const
{
...
double model_visibility_galactic_center_diffractive_scattering_screen::Q(double x, double y) const
{
...
model_visibility_galactic_center_diffractive_scattering_screen::_P_phi::_P_phi(std::string axis, std::string model, double alpha, double phi0)
...
double model_visibility_galactic_center_diffractive_scattering_screen::_P_phi::operator()(double phi) const
{
...
void model_visibility_galactic_center_diffractive_scattering_screen::generate_model(std::vector<double> parameters)
{
  _model.generate_model(parameters);
}

void model_visibility_galactic_center_diffractive_scattering_screen::set_mpi_communicator(MPI_Comm comm)
{
  _model.set_mpi_communicator(comm);
}

std::complex<double> model_visibility_galactic_center_diffractive_scattering_screen::visibility(datum_visibility& d, double acc)
{
  std::complex<double> scatt_kernel(std::exp(-0.5*Dphi(d.u/(1+_M),d.v/(1+_M))),0.0);
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  //if ( rank == 0 )
  //std::cerr << std::setw(15) << d.u << std::setw(15) << d.v << std::setw(15)<< scatt_kernel.real() << std::endl;
  return scatt_kernel*_model.visibility(d,acc);
