...
model_image_shearing_spot_johannsen::model_image_shearing_spot_johannsen(double start_obs, double tobs, std::string sed_fit_parameter_file, double frequency, double M, double D)
...
void model_image_shearing_spot_johannsen::open_error_streams()
{
#if 0
  if (_merr.is_open())
    _merr.close();
...
void model_image_shearing_spot_johannsen::set_image_resolution(int Nray)
{
  _Nray=Nray;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_shearing_spot_johannsen: Rank " << rank << " using image resolution " << _Nray << std::endl;
}

void model_image_shearing_spot_johannsen::set_screen_size(double Rmax)
{
	_Rmax = Rmax;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_shearing_spot_johannsen: Rank " << rank << " using screen size " << _Rmax << std::endl;

}

void model_image_shearing_spot_johannsen::add_background_riaf()
{
  _bkgd_riaf=true;
  
  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_shearing_spot_johannsen: Rank " << rank << " added backgroud sed_fitted_riaf " << std::endl;
}

std::string model_image_shearing_spot_johannsen::model_tag() const
{
  std::stringstream tag;
  tag << "model_image_shearing_spot_johannsen " << _start_obs << " " << _tobs << " " << _frequency << " " << _M << " " << _D;
    return tag.str();
}


void model_image_shearing_spot_johannsen::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  int myrank;
  MPI_Comm_rank(_comm,&myrank);
  Themis::StopWatch sw;
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_shearing_spot_johannsen::generate_image started at: ";
  ...
  const double alpha13 = parameters[2];
  const double alpha22 = parameters[3];
  const double alpha52 = parameters[4];
  const double epsilon = parameters[5];
  const double gbeta = parameters[6];
  
  //Spot density factor
  const double nspot = parameters[7]; //Density factor for renormalized image
  //Spot width
	const double rspot = parameters[8];
	//Initial spot location
	const double t0 = parameters[9]-2000; //Spot start time measured in M
	...
	const double r0 = parameters[10];
	const double outer_radius = r0 + rspot*20.0;
	const double phi0 = parameters[11];
	//Accretion flow parameters
	const double alphaR = parameters[12];
	const double subKep = parameters[13];

	//for ( int i = 0; i < parameters.size(); i++)
	//	std::cerr << std::setw(15) << parameters[i];
	//std::cerr << std::endl;
  ...
  // Choose metric
  VRT2::Johannsen g(1.0,a, alpha13,alpha22,alpha52,epsilon,gbeta);
  ...
  VRT2::AFV_InflowJohannsen afv(g,risco, alphaR, subKep);
  
  // CHOOSE RADIATIVE TRANSFER EFFECTS
  std::vector<VRT2::RadiativeTransfer*> rts;
  VRT2::RT_PP_PA rt_geometric(g);
	...
	if (_bkgd_riaf)
  {
	...
	VRT2::ED_shearing_infall_spot_johannsen ne_spot(g, nspot, rspot, risco, 
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_shearing_spot_johannsen::generate_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
