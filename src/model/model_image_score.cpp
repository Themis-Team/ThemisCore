...
namespace Themis {
  ...
  model_image_score::model_image_score(std::string image_file_name, std::string README_file_name, bool reflect_image, int window_function)
  ...
  std::string model_image_score::model_tag() const
  {
    std::stringstream tag;
    tag << "model_image_score " << _MoD << " " << _image_file_name << " " << _README_file_name << " " << _reflect_image;
    return tag.str();
  }
  
  void model_image_score::generate_model(std::vector<double> parameters)
  {
    _V00 = parameters[0]; // Total intensity in Jy
    _mod = parameters[1]/_MoD; // Rescale factor, parameters[1] is M/D in uas
    _position_angle = parameters[2]; // position angle in radians
  }

  std::complex<double> model_image_score::visibility(datum_visibility& d, double acc)
  {
    ...
    return (model_image::visibility(dtmp,acc)*_V00);
  }

  double model_image_score::visibility_amplitude(datum_visibility_amplitude& d, double acc)
  {
    ...
    return ( model_image::visibility_amplitude(tmp,acc)*_V00 );
  }

  double model_image_score::closure_phase(datum_closure_phase& d, double acc)
  {
  ...
  double model_image_score::closure_amplitude(datum_closure_amplitude& d, double acc)
  {
    datum_visibility tmp1(d.u1,d.v1,std::complex<double>(0.0,0.0),std::complex<double>(d.err,d.err),d.frequency,d.tJ2000,d.Station1,d.Station2,d.Source);
    datum_visibility tmp2(d.u2,d.v2,std::complex<double>(0.0,0.0),std::complex<double>(d.err,d.err),d.frequency,d.tJ2000,d.Station2,d.Station3,d.Source);
  ...
  void model_image_score::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
  {
  }
}
