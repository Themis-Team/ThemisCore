/*!
  \file model_polarized_image_general_riaf_waxion.h
  \author Avery E. Broderick, Zhiren Wang
  \date  August 2019
  \brief Header file for the extended RIAF image model class with mass scaling and an axion cloud.
  \details To be added
*/

#ifndef Themis_MODEL_POLARIZED_IMAGE_GENERAL_RIAF_WAXION_H_
#define Themis_MODEL_POLARIZED_IMAGE_GENERAL_RIAF_WAXION_H_
...
/*!
  ...
  \details Provides explicit implementation of the model_polarized_image object for the semi-analytic RIAF models.  There are additional tuning parameters that may impact accuracy, e.g., distance to the image screen, number of rays to produce, etc.  See the appropriate parts of model_image_riaf.cpp for more.  It also provides a flux estimate for inclusion in later flux_model extensions (model_riaf).
  ...
  - parameters[13] .. Axion mass in eV.
  - parameters[14] .. Dimensionless axion coupling (assuming saturation).
  - parameters[15] .. Position angle (in model_image) in radians.

 \warning This defines a model_polarized_image object associated with a RIAF observed at single frequency.  Requires the VRT2 library to be installed.
*/
class model_polarized_image_general_riaf_waxion : public model_polarized_image
{
 private:
 ...
 public:
  ...
  model_polarized_image_general_riaf_waxion(double frequency=230e9, double D=VRT2::VRT2_Constants::D_SgrA_cm);
  virtual ~model_polarized_image_general_riaf_waxion() {};
  
  //! Returns the number of the parameters the model expects
  virtual inline size_t size() const { return 16; };
  ...
  //! Sets model_polarized_image_general_riaf to generate production images with
  ...
  //! Returns period of axion, if defined
  double axion_period() const {
    return _axion_period;
  };
