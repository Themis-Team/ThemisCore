/*!
  \file model_polarized_image_general_riaf_waxion.cpp
  \author Avery E. Broderick, Zhiren wang
  \date  April, 2017
  \brief Implements the RIAF image model class within an axion cloud.
  \details To be added
*/
...
model_polarized_image_general_riaf_waxion::model_polarized_image_general_riaf_waxion(double frequency, double D)
  : _comm(MPI_COMM_WORLD), _D(D), _frequency(frequency), _Rmax(15.0), _Nray_base(128), _Nray(128), _number_of_refines(0), _axion_period(0)
{
  open_error_streams();
}

void model_polarized_image_general_riaf_waxion::open_error_streams()
{
#if 0
  if (_merr.is_open())
    _merr.close();
...
void model_polarized_image_general_riaf_waxion::set_screen_size(double imgR)
{
  _Rmax=imgR;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_polarized_image_general_riaf_waxion: Rank " << rank << " using screen size " << _Rmax << std::endl;

}
void model_polarized_image_general_riaf_waxion::set_image_resolution(int Nray, int number_of_refines)
{
  _Nray_base=Nray;
  _Nray = _Nray_base;
...
std::string model_polarized_image_general_riaf_waxion::model_tag() const
{
  ...
  tag << "model_polarized_image_general_riaf_waxion " << _frequency << " " << _D;
  return tag.str();
}
  
void model_polarized_image_general_riaf_waxion::generate_polarized_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& Q, std::vector<std::vector<double> >& U, std::vector<std::vector<double> >& V, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  int myrank;
  MPI_Comm_rank(_comm,&myrank);
  Themis::StopWatch sw;
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_polarized_image_general_riaf_waxion_shearing_spot::generate_image started at: ";
  ...
  I.resize(_Nray); Q.resize(_Nray); U.resize(_Nray); V.resize(_Nray); alpha.resize(_Nray); beta.resize(_Nray);
  if (!std::isnan(pmap.I_int()))
  {
    for (int i=0; i<_Nray; i++)
    {
        I[i].resize(_Nray); Q[i].resize(_Nray); U[i].resize(_Nray); V[i].resize(_Nray); alpha[i].resize(_Nray); beta[i].resize(_Nray);
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_polarized_image_general_riaf_waxion::generate_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
...
double model_polarized_image_general_riaf_waxion::generate_flux_estimate(double accuracy, std::vector<double> parameters)
{
  /////////////////////////////////////////////////////////
  ...
  //std::cerr << "model_polarized_image_general_riaf_waxion::generate_flux_estimate : generated first image " << I_new << "\n";
  ...
  do                                  //now we keep refining until certain conditions are met
  {
    ...
    /*std::cerr << "model_polarized_image_general_riaf_waxion::generate_flux_estimate : " << refinement_level
	      << std::setw(15) << I_old 
	      << std::setw(15) << I_new << "\n";*/
...
double model_polarized_image_general_riaf_waxion::estimate_image_size_log_spiral(VRT2::PolarizationMap &pmap, double initial_r, double theta_step, 
							double percent_from_max_I, unsigned int max_iter, double safety_factor)
{
  ...
  while (i < max_iter && !found_size)
  {
    ...
    /*
    std::cerr << "model_polarized_image_general_riaf_waxion::estimate_image_size_log_spiral : "
