...
#ifndef Themis_MODEL_VISIBILITY_AMPLITUDE_H_
#define Themis_MODEL_VISIBILITY_AMPLITUDE_H_
...
/*! 
  \brief Defines the interface for models that generate visibility amplitude data appropriate for comparison with datum_visibility_amplitude.h.

  \details To facilitate model-data comparisons, here a fixed interface that may be assumed by likelihoods, etc., for generating model visibility amplitude data is defined.  All models that produce visibility amplitude data should be child classes of model_visibility_amplitude, thereby inheriting this interface.  The key function is visibility_amplitude which takes a datum_visibility_amplitude, providing access to all of the relevant accoutrements of a data point, and an accuracy parameter, and returns the model visibility amplitude.

  \warning This class contains multiple purely virtual functions, making it impossible to generate an explicit instantiation.
*/
class model_visibility_amplitude 
{
 public:
  model_visibility_amplitude();
  virtual ~model_visibility_amplitude();
  ...
  virtual double visibility_amplitude(datum_visibility_amplitude& d, double accuracy) = 0;
  ...
  virtual void set_mpi_communicator(MPI_Comm comm) {
    //std::cout << "model_visibility_amplitude proc " << MPI::COMM_WORLD.Get_rank() << " set comm" << std::endl;
    _comm=comm;
  };
