/*!
  ...
  \author Paul Tiede
  \date  June, 2018
  \brief Implements SED-fitted RIAF model class with Johannsen metric.
  \details To be added
*/
...
model_image_sed_fitted_riaf_johannsen::model_image_sed_fitted_riaf_johannsen(std::string sed_fit_parameter_file, double frequency)
...
void model_image_sed_fitted_riaf_johannsen::open_error_streams()
{
#if 0
  if (_merr.is_open())
    _merr.close();
...
void model_image_sed_fitted_riaf_johannsen::use_small_images()
{
  set_image_resolution(32);
}

void model_image_sed_fitted_riaf_johannsen::set_image_resolution(int Nray)
{
  _Nray=Nray;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_image_sed_fitted_riaf_johannsen: Rank " << rank << " using image resolution " << _Nray << std::endl;
}

std::string model_image_sed_fitted_riaf_johannsen::model_tag() const
{
  std::stringstream tag;
  tag << "model_image_sed_fitted_riaf_johannsen " << _frequency << " " << _M << " " << _D;
  return tag.str();
}

double model_image_sed_fitted_riaf_johannsen::generate_renormalized_image(double density_factor, int Nrays, std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  /*
  int myrank, mysize;
  MPI_Comm_rank(_comm,&myrank);
  MPI_Comm_size(_comm,&mysize);
  std::cout << "model_image_sed_fitted_riaf_johannsen::generate_renormalized_image"
	    << std::setw(5) << myrank
  ...
  */
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "model_image_sed_fitted_riaf_johannsen::generate_renormalized_image started at: ";
  ...
  VRT2::AFV_InflowJohannsen afv(g,risco, 0, 1); // switches to free-fall at ISCO
  //VRT2::AFV_Keplerian afv(g,risco);
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "model_image_sed_fitted_riaf_johannsen::generate_renormalized_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
...
void model_image_sed_fitted_riaf_johannsen::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  /*
  std::cout << "model_image_sed_fitted_riaf_johannsen::generate_image"
  ...
  */
  ...
  Themis::StopWatch sw;
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_sed_fitted_riaf_johannsen::generate_image started at: ";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
    sw.start();
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_image_sed_fitted_riaf_johannsen::generate_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
