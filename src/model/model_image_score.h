...
#ifndef THEMIS_MODEL_IMAGE_SCORE_H_
#define THEMIS_MODEL_IMAGE_SCORE_H_
...
namespace Themis {
  ...
  class model_image_score : public model_image
  {
    
  public:
    
    //model_image_score(int Nray, double M, double D, double fov, double frequency);
    // Takes file names
    model_image_score(std::string image_file_name, std::string README_file_name, bool reflect_image=false, int window_function=0);
    //model_image_score(std::string image_file_name, double fovx, double fovy, double frequency=230e9);
    
    virtual ~model_image_score() {};
  ...
  private:
    ...
    double _mod, _V00; // M/D ratio, total flux in Jy       
