/*!
  \file model_polarized_image_gaussian_spot.cpp
  \author Paul Tiede
  \date  March, 2018
  \brief Implements gaussian spot image class.
*/
...
model_polarized_image_gaussian_spot::model_polarized_image_gaussian_spot(
    double start_obs, double tobs, bool bkgd_riaf, double frequency, double M, double D)
  : _comm(MPI_COMM_WORLD),
    _start_obs(start_obs), 
    _tobs(tobs), 
    _bkgd_riaf(bkgd_riaf), 
    _frequency(frequency),
     _M(M), _D(D), 
     _Nray(128), _Nray_base(128), _number_of_refines(0), _Rmax(15)
{
  open_error_streams();
}

void model_polarized_image_gaussian_spot::open_error_streams()
{
#if 0
  if (_merr.is_open())
    _merr.close();
...
std::string model_polarized_image_gaussian_spot::model_tag() const
{
  std::stringstream tag;
  tag << "model_polarized_image_gaussian_spot " << _start_obs << " " << _tobs << " " << _frequency << " " << _M << " " << _D;
    return tag.str();
}

void model_polarized_image_gaussian_spot::set_image_resolution(int Nray, int number_of_refines)
{
  _Nray_base=Nray;
  _Nray = _Nray_base;
...
void model_polarized_image_gaussian_spot::set_screen_size(double Rmax)
{
	_Rmax = Rmax;

  int rank;
  MPI_Comm_rank(_comm,&rank);
  std::cout << "model_polarized_image_gaussian_spot: Rank " << rank << " using screen size " << _Rmax << std::endl;

}


void model_polarized_image_gaussian_spot::generate_polarized_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& Q, std::vector<std::vector<double> >& U, std::vector<std::vector<double> >& V, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
{
  int myrank;
  MPI_Comm_rank(_comm,&myrank);
  Themis::StopWatch sw;
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_polarized_image_gaussian_spot::generate_image started at: ";
  ...
  if (myrank==0 && _merr.is_open()) {
    _merr << "------------ model_polarized_image_gaussian_spot::generate_image finished at:";
    for (size_t j=0; j<parameters.size(); ++j)
      _merr << std::setw(15) << parameters[j];
    _merr << std::endl;
