/*!
  ...
  \date  June, 2017
  \brief Header file for the asymmetric Gaussian image class.
  \details To be added
*/

#ifndef Themis_MODEL_IMAGE_ASYMMETRIC_GAUSSIAN_H_
#define Themis_MODEL_IMAGE_ASYMMETRIC_GAUSSIAN_H_
...
namespace Themis {
  ...
  Parameter list:\n
    ...
    - parameters[1] ... \f$ \sigma = \sqrt{2} \sigma_m \sigma_M / \left( \sigma_M^2 + \sigma_m^2 \right)^{1/2} \f$
    - parameters[2] ... \f$ A = (\sigma_M^2-\sigma_m^2)/(\sigma_M^2+\sigma_m^2) \f$ 
  ...
  */
  class model_image_asymmetric_gaussian : public model_image
  {
    private:
    ...
    public:
    model_image_asymmetric_gaussian();
    virtual ~model_image_asymmetric_gaussian() {};
  ...
  virtual std::complex<double> visibility(datum_visibility& d, double accuracy);
    ...
    {
      return "model_image_asymmetric_gaussian";
    };
