...
namespace Themis {
  ...
  model_image_mring::model_image_mring(size_t order)
    : _order(order), _an(_order, 0.0), _bn(_order, 0.0), _Nray(128), _use_analytical_visibilities(true)
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "Creating model_image_mring in rank " << world_rank << std::endl;
  }

  void model_image_mring::use_numerical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_mring::use_analytical_visibilities()
  {
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  ...
  void model_image_mring::set_image_resolution(int Nray)
  {
    _Nray = Nray;
    
    int world_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    std::cout << "model_image_mring: Rank " << world_rank << " using image resolution " << _Nray << std::endl;
  }


void model_image_mring::generate_model(std::vector<double> parameters)
{
  ...
  void model_image_mring::generate_image(std::vector<double> parameters, std::vector<std::vector<double> >& I, std::vector<std::vector<double> >& alpha, std::vector<std::vector<double> >& beta)
  {
    int Ntot = _Nray;
  ...
  double model_image_mring::BesselJ0(double x)
  {
    double ax,z;
  ...
  double model_image_mring::BesselJ1(double x)
  {
    double ax,z;
    double xx,y,ans,ans1,ans2;
  ...
  double model_image_mring::BesselJ(int n, double x)
  {
  ...
  std::complex<double> model_image_mring::complex_visibility(double u, double v)
    {
  ...
  double model_image_mring::visibility_amplitude(datum_visibility_amplitude& d, double acc)
  {
  ...
  double model_image_mring::closure_phase(datum_closure_phase& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  double model_image_mring::closure_amplitude(datum_closure_amplitude& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
  ...
  std::complex<double> model_image_mring::visibility(datum_visibility& d, double acc)
  {
    if (_use_analytical_visibilities)
    {
