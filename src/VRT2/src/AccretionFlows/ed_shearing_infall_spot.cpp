#include "ed_shearing_infall_spot.h"

namespace VRT2 {

ED_shearing_infall_spot::ED_shearing_infall_spot( Metric& g, double density_scale, double rSpot, double ri, AFV_ShearingInflow& afv, double infallRate, double subKep, double outer_radius, double t0, double r0, double theta0, double phi0)
...
double ED_shearing_infall_spot::get_density(const double t, const double r, const double theta, const double phi)
{
  if (!_table_gen)
  {
...
double ED_shearing_infall_spot::infall_density(const double t, const double r, const double theta, const double phi)
{
  ...
  _g.reset(xspot0[0],xspot0[1],M_PI/2.0,xspot0[3]);
  FourVector<double> u0 = _afv(xspot0[0],xspot0[1],M_PI/2.0,xspot0[3]);
  ...
  double rho = detgRatio*_density_scale*
               FastMath::exp(-dr*dr/(2.*_rspot*_rspot))*utRatio;
...
double ED_shearing_infall_spot::delta_r(const double t, const double r, const double theta, const double phi)
{
...
void ED_shearing_infall_spot::generate_spot_position_table()
{
...
std::valarray<double>& ED_shearing_infall_spot::curve_now(const double r)
{
...
std::valarray<double>& ED_shearing_infall_spot::integral_curve(double t)
{
...
double ED_shearing_infall_spot::reparametrize( const double y[])
{
...
void ED_shearing_infall_spot::derivs(double x, const double y[], double dydx[])
{
...
int ED_shearing_infall_spot::rkqs(double y[], double dydx[], int n, double& x, double htry, 
...
void ED_shearing_infall_spot::rkck(double y[], double dydx[], 
...
void ED_shearing_infall_spot::generate_table()
{
...
void ED_shearing_infall_spot::set_table(std::vector<double> spot_tables[4])
{
  for ( int i = 0; i < 4; ++i){
    _xspot_tables[i].resize(0);
    _xspot_tablesRev[i].resize(0);
...
void ED_shearing_infall_spot::export_table(std::vector<double> spot_tables[4])
{
  for ( int i = 0; i < 4; i++)
