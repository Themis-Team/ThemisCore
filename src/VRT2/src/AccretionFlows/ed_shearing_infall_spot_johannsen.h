/*!
  ...
  \details Calculates the density profile for an initially gaussian spot falling into a Johannsen black hole. Our velocity vector field is given by a combination of keplerian and free-fall velocity field. Does this by integrating one curve for the spot and the uses the azimuthal and time symmetry of the Johannsen metric to find all the curves. To calulcate densiy we compute the density expansion factor in a table in the constructor of the class. We the perform a linear intepolation to find the correct values.
*/
#ifndef VRT2_ED_SHEARING_INFALL_SPOT_JOHANNSEN_H
#define VRT2_ED_SHEARING_INFALL_SPOT_JOHANNSEN_H
...
class ED_shearing_infall_spot_johannsen : public ElectronDensity
{
 public:
  /*
  ...
  */
  ...
  ED_shearing_infall_spot_johannsen( Johannsen& g, double density_scale, double rSpot, double ri, AFV_InflowJohannsen& afv, double infallRate, double subKep, double outer_radius, double t0, double r0, double theta0, double phi0);

  virtual ~ED_shearing_infall_spot_johannsen() {};
 ...
 protected:
  ...
  AFV_InflowJohannsen& _afv; //Accretion flow velocity field
...
inline void ED_shearing_infall_spot_johannsen::get_yscal(double h, double x, const double y[], const double dydx[], double yscal[]) const
{
