...
std::valarray<double>& RT_Axion::IQUV_abs(const double iquv[], const double dydx[])
{
  ...
  double _delta_ar_4rg_p = Real_a(_x.con(0), 4 + 1, _theta_test, _phi_test);
  double _delta_ar_4rg_m = Real_a(_x.con(0), 4 - 1, _theta_test, _phi_test);
  double _delta_ar_4rg_cd = (_delta_ar_4rg_p - _delta_ar_4rg_m) / 2;

  double _dadr_7rg = dadr(_x.con(0), _r_test, _theta_test, _phi_test);

  double _delta_ar_7rg_p = Real_a(_x.con(0), _r_test + 1, _theta_test, _phi_test);
  double _delta_ar_7rg_m = Real_a(_x.con(0), _r_test - 1, _theta_test, _phi_test);
