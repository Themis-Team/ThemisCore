...
// Only include once
#ifndef VRT2_RT_POWER_LAW_SYNCHROTRON_H
#define VRT2_RT_POWER_LAW_SYNCHROTRON_H
...
namespace VRT2 {
class RT_PowerLawSynchrotron : public RadiativeTransfer
{
 public:
  // Constructor
  RT_PowerLawSynchrotron(Metric& g,
			 ElectronDensity& ne, AccretionFlowVelocity& u, MagneticField& B,
			 double spectral_index, double gamma_min);
  RT_PowerLawSynchrotron(const double y[], Metric& g,
			 ElectronDensity& ne, AccretionFlowVelocity& u, MagneticField& B,
			 double spectral_index, double gamma_min);
  RT_PowerLawSynchrotron(FourVector<double>& x, FourVector<double>& k, Metric& g,
			 ElectronDensity& ne, AccretionFlowVelocity& u, MagneticField& B,
			 double spectral_index, double gamma_min);
  virtual ~RT_PowerLawSynchrotron() { };
