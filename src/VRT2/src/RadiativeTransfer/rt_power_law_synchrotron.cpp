#include "rt_power_law_synchrotron.h"
#include "fast_math.h"

namespace VRT2 {
RT_PowerLawSynchrotron::RT_PowerLawSynchrotron(Metric& g,
...
RT_PowerLawSynchrotron::RT_PowerLawSynchrotron(const double y[], Metric& g,
					       ElectronDensity& ne, AccretionFlowVelocity& u, MagneticField& B,
					       double spectral_index, double gamma_min)
...
RT_PowerLawSynchrotron::RT_PowerLawSynchrotron(FourVector<double>& x, FourVector<double>& k, Metric& g,
					       ElectronDensity& ne, AccretionFlowVelocity& u, MagneticField& B,
					       double spectral_index, double gamma_min)
...
void RT_PowerLawSynchrotron::set_frequency_scale(double omega0)
{
  RadiativeTransfer::set_frequency_scale(omega0);
  set_constants();
}

void RT_PowerLawSynchrotron::set_length_scale(double L)
{
  RadiativeTransfer::set_length_scale(L);
  set_constants();
}

void RT_PowerLawSynchrotron::reinitialize(const double y[])
{
  _x.mkcon(y);
  _k.mkcov(y+4);
  set_common_funcs();
...
void RT_PowerLawSynchrotron::reinitialize(FourVector<double>& x, FourVector<double>& k)
{
  _x = x;
  _k = k;
  set_common_funcs();
...
void RT_PowerLawSynchrotron::set_constants()
{
...
void RT_PowerLawSynchrotron::set_common_funcs()
{
  _n0 = _Cn * _ne(_x);
...
void RT_PowerLawSynchrotron::get_Stokes_alignment_angle(FourVector<double>& u, FourVector<double>& b, double& cs, double& sn)
{
...
double RT_PowerLawSynchrotron::isotropic_absorptivity(const double dydx[])
{
  if (_omegaB > 0.0)
...
std::valarray<double>& RT_PowerLawSynchrotron::IQUV_abs(const double iquv[], const double dydx[])
{
  ...
  if (_omegaB > 0.0) {
    ...
    if (_omega>omega_min)
      //abs = consts/_omega * std::pow(_omegaB*_sn_alpha/_omega,_spectral_index+2.5-1);
...
std::valarray<double>& RT_PowerLawSynchrotron::IQUV_ems(const double dydx[])
{
  if (_omegaB > 0.0) {
...
double RT_PowerLawSynchrotron::dl_dlambda(const double dydx[])
{
...
double RT_PowerLawSynchrotron::gammln(double xx)
{
  double x,y,tmp,ser;
...
void RT_PowerLawSynchrotron::dump(std::ostream& dout, double dydx[])
{
  double ems, abs;
  if (_omegaB > 0.0) {
