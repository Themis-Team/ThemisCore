/*!
  ...
  \date Dec, 2017
  \brief Example demonstrating how to drive the external radiative transfer code RAPTOR

  \details See model_image_raptor for details.
*/
#define VERBOSITY (0)
...
int main(int argc,char* argv[])
{
  ...
  // Themis::model_image_raptor RAPTOR_IMAGE;
  Themis::model_image_raptor image;
  // RAPTOR_IMAGE.generate_model(p);
  // RAPTOR_IMAGE.get_image(aN,bN,IN);
  image.generate_model(p);
  image.get_image(aN,bN,IN);

  // RG: Need to turn off scattering off when using closure_phase data 
  // Themis::model_ensemble_averaged_scattered_image image(RAPTOR_IMAGE);
  ...
  MC_obj.run_sampler(L_obj,  
                      ...
                      "Chain-raptor-VM-5T.dat", "Lklhd-raptor-VM-5T.dat", 
		     "Chi2-raptor-VM-5T.dat", means, ranges, var_names, false, 0); 
