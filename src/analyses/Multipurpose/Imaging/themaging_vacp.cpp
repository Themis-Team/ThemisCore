...
int main(int argc, char* argv[])
{
  ...
  if (vm_file!="")
  {
    ...
    if (Reconstruct_gains) 
    {
      ...
      if (gain_high_file!="")
      {
	lva_hi->read_gain_file(gain_high_file);
	lva_hi->fix_gains();
      }
    }
    else
    ...
    if (Reconstruct_gains) 
    {
      ...
      if (gain_low_file!="")
      {
	lva_lo->read_gain_file(gain_low_file);
	lva_lo->fix_gains();
      }
    }
    else
  ...
  */
  ...
  if (preoptimize_flag)
  {
    ...
    means = opt_obj.run_optimizer(L_obj, VM_data_hi.size()+VM_data_lo.size()+CP_data.size(), means, "PreOptimizeSummary.dat",opt_instances);
  ...
  for (size_t rep=0; rep<Number_of_reps; ++rep)
  {
    ...
    if (postoptimize_flag)
    {
      opt_obj.set_cpu_distribution(Number_of_procs_per_lklhd);
      opt_obj.set_kickout_parameters(opt_ko_llrf,opt_ko_itermax,opt_ko_rounds);
      pbest = opt_obj.run_optimizer(L_obj, VM_data_hi.size()+VM_data_lo.size()+CP_data.size(), pbest, "PostOptimizeSummary.dat",opt_instances);
    }
