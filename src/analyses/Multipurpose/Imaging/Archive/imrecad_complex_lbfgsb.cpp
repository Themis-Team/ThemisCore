/*!
  \file analyses/Imaging/imrecad_complex_lbfgsb.cpp
...
int main(int argc, char* argv[])
{
  ...
  size_t opt_instances = world_size;
  ...
  for (int k=1; k<argc;)
  {
	...
	seed = atoi(argv[k++]);
      ...
      {
	if (world_rank==0)
	  std::cerr << "ERROR: An int argument must be provided after --need.\n";
	std::exit(1);
      }
    }
    ...
    {
      if (k<argc)
	opt_instances = std::max((size_t)atoi(argv[k++]), opt_instances);
      else
      {
	if (world_rank==0)
  ...
  //Now lets set the initial locations
  //Themis::Ran2RNG rng(seed+world_rank);
  //Themis::GaussianRandomNumberGenerator<Ran2RNG> grng(seed+work_rank*342);
  std::vector<std::vector<double> > starting_points;
  starting_points.push_back(means);
  Themis::Ran2RNG urng(seed+100+world_rank);
  Themis::GaussianRandomNumberGenerator<Themis::Ran2RNG> grng(seed+200+world_rank*42);
  for (size_t i = 1; i < opt_instances; ++i)
  {
    ...
    double Itot=0.0;
    double Imeans=0.0;
    double fovx = std::max(P[image_ptr->size()-3]->lower_bound(),std::min(P[image_ptr->size()-3]->upper_bound(),Field_of_view_x*(1.0+0.25*urng.rand())));
    double fovy = std::max(P[image_ptr->size()-2]->lower_bound(),std::min(P[image_ptr->size()-2]->upper_bound(),Field_of_view_y*(1.0+0.25*urng.rand())));
    double dA = fovx*fovy/double((Number_of_pixels_x-1)*(Number_of_pixels_y-1));
    for (size_t j=0; j<image_ptr->size()-3; ++j)
    {
      pguess.push_back(1.0*grng.rand()+means[j]); // Imposes an envelope of the "means"
      Itot += std::exp(pguess[j]) * dA; 
      Imeans += std::exp(means[j]) * dA; 
    }

    for (size_t j=0; j<image_ptr->size()-3; ++j)
      pguess[j] += std::log(Imeans/Itot);

    
    pguess.push_back(fovx);
    pguess.push_back(fovy);
    if (Position_angle==-999)
      pguess.push_back(0.5*M_PI*urng.rand()-0.25*M_PI);
    else
    {
      double PAguess = Position_angle + 0.1*grng.rand();
      PAguess = std::fmod(PAguess-P[image_ptr->size()-1]->lower_bound()+P[image_ptr->size()-1]->upper_bound()-P[image_ptr->size()-1]->lower_bound(),P[image_ptr->size()-1]->upper_bound()-P[image_ptr->size()-1]->lower_bound()) + P[image_ptr->size()-1]->lower_bound();
      pguess.push_back(PAguess);
    }
    
    // Other elements
    for (size_t j=image_ptr->size(); j<image.size(); ++j)
      pguess.push_back(P[j]->lower_bound() + (P[j]->upper_bound()-P[j]->lower_bound())*(0.01 + 0.98*urng.rand()));

    starting_points.push_back(pguess);
  }

  if (world_rank==0)
  {
    std::ofstream spout("starting_points.txt");
    spout << "#chainfmt stan\n#\n";
    for (size_t i=0; i<starting_points.size(); ++i)
    {
      for (size_t j=0; j<starting_points[i].size(); ++j)
	spout << std::setw(15) << starting_points[i][j];
      spout << std::endl;
    }
  }
  ...
  optim.set_start_points(starting_points); 
