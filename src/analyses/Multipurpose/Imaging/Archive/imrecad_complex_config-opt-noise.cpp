...
int main(int argc, char* argv[])
{
  ...
  int Ckpt_frequency = 40; // per swaps
  int verbosity = 0;
    ...
    */
	...
	std::cerr << "NAME\n"
		  ...
		  << "\t\tNormalize the image by the light curve in file specified.  If not set, no light curve is set.  Default: none.\n"
	          << "\t-n, --model-noise\n"
		  << "\t\tIncludes a noise model (currently broken power change).\n"
  ...
  {  
    ...
    // Position angle
    P.push_back(new Themis::prior_linear(-M_PI/2.,0));
  ...
  // if ((model_noise) && (Number_start_params < Number_of_pixels_x*Number_of_pixels_y+5)) { // RG: reset the noise parameters -p only covers themage parameters, still assume we only themage (without nuisance Gaussians, rings etc)...
    ...
    if (model_noise_priors_sgra && nc_file == "") {
	...
	means.push_back(start_parameter_list[k]);
        ...
        double noise_upper[4];
        double noise_lower[4];
        double noise_med[4];
        //Set fractional and that stuff
        P.push_back(new Themis::prior_linear(0.,0.02));
        //P.push_back(new Themis::prior_linear(0.,0.0001)); // Debugging by removing scattering floor
        P.push_back(new Themis::prior_linear(0.,0.05));
        //P.push_back(new Themis::prior_linear(0.,0.0001)); // Debugging by removing sys err floorr
        //Now read in the config file for the others
        if (world_rank == 0){
            std::vector<std::vector<double> > params = read_config(nc_file);
            for ( int nn = 1; nn < 4; ++nn ){
                noise_lower[nn] = params[nn][0]; //0.5% 
                noise_upper[nn] = params[nn][6]; //99.5%
                noise_med[nn] = params[nn][3];
            }
            //Now set the a amplitude since we constraint          
            noise_lower[0] = params[4][0];
            noise_upper[0] = params[4][6];
            noise_med[0] = params[4][3];
        }
        //Now broadcast to everyone
        MPI_Bcast(&noise_lower[0], 4, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&noise_upper[0], 4, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&noise_med[0], 4, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        //Now set the priors
        for ( size_t nt = 0; nt < 4; ++nt ){
            P.push_back(new Themis::prior_linear(noise_lower[nt], noise_upper[nt]));
        }

        if (Number_start_params < start_Number_of_pixels_x*start_Number_of_pixels_y+5) {
          means.push_back(0.01); // noise threshold
	  means.push_back(0.01); // fractional (mimicking non-closing errors)
	  means.push_back(noise_med[0]); // bpc noise amplitude at 4Glambda
	  means.push_back(noise_med[1]); // uv distance where two powerlaws break
  ...
  if (preoptimize_flag)
  {
    ...
    // // int seed = 23;                                                                              // optim.parallel_optimizer(parameters, lmap, number_instances, seed, "");
