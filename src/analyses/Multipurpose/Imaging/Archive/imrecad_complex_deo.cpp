/*!
  \file analyses/Imaging/imrecad_complex_deostan.cpp
...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
    ...
    */
    ...
    else if (opt == "--sampler")
    {
      ...
      {
        if (world_rank == 0)
          std::cerr << "ERROR: A string argument must be provided after --sampler\n";
      }
    }
  ...
  std::shared_ptr<Themis::sampler_MCMC_base> sampler;
  int round_start = 0;
  //Create the tempering sampler which is templated off of the exploration samples
  if (sampler_type == "diag"){
    std::shared_ptr<Themis::sampler_deo_tempering_MCMC<Themis::sampler_stan_adapt_diag_e_nuts_MCMC> > 
      DEO(new Themis::sampler_deo_tempering_MCMC<Themis::sampler_stan_adapt_diag_e_nuts_MCMC>(seed, L_temp, var_names, means.size()));
    sampler = DEO;

    //Set the output stream which really just calls the hmc output steam.
    //The exploration sampler handles all the output.
    DEO->set_output_stream("chain.dat", "state.dat", "diag_summary.dat");
  ...
  //Now we can also change some options for the sampler itself.
    ...
    if (restart_flag)
    {
      ...
      std::shared_ptr<Themis::sampler_deo_tempering_MCMC<Themis::sampler_automated_factor_slice_sampler_MCMC> > 
        DEO(new Themis::sampler_deo_tempering_MCMC<Themis::sampler_automated_factor_slice_sampler_MCMC>(seed, L_temp, var_names, means.size()));
    sampler = DEO;

    //Set the output stream which really just calls the hmc output steam.
    //The exploration sampler handles all the output.
    DEO->set_output_stream("chain.dat", "state.dat", "slice_summary.dat");
  ...
  } else if (sampler_type == "dense") {  
    std::shared_ptr<Themis::sampler_deo_tempering_MCMC<Themis::sampler_stan_adapt_dense_e_nuts_MCMC> > 
        DEO(new Themis::sampler_deo_tempering_MCMC<Themis::sampler_stan_adapt_dense_e_nuts_MCMC>(seed, L_temp, var_names, means.size()));
    sampler = DEO;
    //Set the output stream which really just calls the hmc output steam.
    //The exploration sampler handles all the output.
    DEO->set_output_stream("chain.dat", "state.dat", "dense_summary.dat");
  ...
  //Now we can also change some options for the sampler itself.
    ...
    std::cerr << sampler_type << " not found please use one of diag, dense, slice\n";
    std::exit(1);
  }
  ...
  for (size_t rep=round_start; rep<=Number_of_reps; ++rep)
  {
	...
	std::cerr << "Starting MCMC on round " << rep << std::endl;
      sampler->run_sampler( 1, thin_factor, refresh_rate, verbosity);
      clock_t end = clock();
      if (world_rank == 0)
