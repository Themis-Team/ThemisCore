/*!
  \file analyses/Imaging/imrecad_complex_single.cpp
  \author
  \date Feb 2020
...
int main(int argc, char* argv[])
{
  ...
  */
  ...
  int num_rounds = number_of_rounds; //number of adaptation rounds.
  ...
  int num_swaps_sample = Number_of_steps;
  ...
  int rep = 0;
  {
    ...
    chain_file_name << "chain_temistanv5_" << std::setfill('0') << std::setw(3) << rep << ".dat";
    anneal_file_name << "annealing_v5" << std::setfill('0') << std::setw(3) << rep << ".dat";
    lklhd_file_name << "state_themistanv5_" << std::setfill('0') << std::setw(3) << rep << ".dat";
    stansum_file_name << "stan_summary_deov5_" << std::setfill('0') << std::setw(3) << rep << ".dat";

    // Set some output for summary
    //DEO.set_sampler_summary(stansum_file_name.str());
    
    //Set the output stream which really just calls the hmc output steam.
    //The exploration sampler handles all the output.
    DEO.set_output_stream(chain_file_name.str(),lklhd_file_name.str(),stansum_file_name.str()); //"chain_themistanv5.dat", "state_themistanv5.dat");
    
    //Sets the output for the annealing summary information
    DEO.set_annealing_output(anneal_file_name.str()); //"annealing_v5.dat");
    ...
    std::cerr << "FOO " << Lval << std::endl;
    //if (Reconstruct_gains)
    //  lv_hi->fix_gains();
    Lval = L_obj(pbest);
    std::cerr << "BAR " << Lval << std::endl;
    ...
    //lv_hi->solve_for_gains_during_gradient();
    //lv_hi->fix_gains_during_gradient();
    //lv_hi->fix_gains();

    // Adapt sampler
    //hmc->set_adaptation_parameters( num_swamps_adapt, sav_warmup);
    hmc->set_adaptation_parameters( num_swamps_adapt, save_warmup);
    
    // Run the sampler with the given settings
    std::cout << "Running the sampler!\n";
    ...
    /*
    // Process final output (MIGHT FAIL IF NAMING DOESN'T WORK THE SAME AS BEFORE)
    std::vector<double> pbest = DEO.find_best_fit(chain_file_name.str()+".r"+std::to_string(number_of_rounds-1),lklhd_file_name.str()+".r"+std::to_string(number_of_rounds-1));

    if (postoptimize_flag)
    {
      opt_obj.set_cpu_distribution(Number_of_procs_per_lklhd);
      opt_obj.set_kickout_parameters(opt_ko_llrf,opt_ko_itermax,opt_ko_rounds);
      pbest = opt_obj.run_optimizer(L_obj, V_data_hi.size(), pbest, "PostOptimizeSummary.dat",opt_instances);
    }
