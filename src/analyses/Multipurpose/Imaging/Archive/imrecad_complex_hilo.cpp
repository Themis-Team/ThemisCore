/*!
  \file analyses/Imaging/imrecad_complex_hilo.cpp
  \author
  \date Feb 2020
...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
    ...
    */
	...
	std::cerr << "NAME\n"
		  ...
		  << "SYNOPSIS"
		  << "\tmpirun -np 40 imrecad_hilo -v v_file_list [OPTIONS]\n\n"
		  << "REQUIRED OPTIONS\n"
  ...
  // Read in data files
  Themis::data_visibility V_data_hi, V_data_lo;
  std::vector<Themis::likelihood_base*> L;
  Themis::likelihood_optimal_complex_gain_visibility *lv_hi=0, *lv_lo=0;
  ...
  if (Reconstruct_gains) 
  {
    ...
    L.push_back(new Themis::likelihood_visibility(V_data_hi,image));
  ...
  if (Reconstruct_gains) 
  {
    lv_lo = new Themis::likelihood_optimal_complex_gain_visibility(V_data_lo,image,station_codes,station_gain_priors);
    L.push_back(lv_lo);
    if (gain_low_file!="")
    {
    ...
    L.push_back(new Themis::likelihood_visibility(V_data_lo,image));
  vin.close();
