...
int main(int argc, char* argv[])
{
  ...
  std::string lc_file="";
  bool uncertainty_loose_change = false;
  bool uncertainty_broken_power_change = false;  
  ...
  bool uncertainty_loose_change_start = false;
  bool uncertainty_broken_power_change_start = false;
  ...
  for (int k=1; k<argc;)
  {
    ...
    else if (opt=="-ulc" || opt=="--uncertainty-loose-change")
    {
      uncertainty_loose_change=true;
    }    
    else if (opt=="-ubpc" || opt=="--uncertainty-broken-power-change")
    {
      uncertainty_broken_power_change=true;
    }    
    ...
    else if (opt=="-ulc-start" || opt=="--uncertainty-loose-change-start")
    {
      uncertainty_loose_change_start=true;
    }    
    else if (opt=="-ubpc-start" || opt=="--uncertainty-broken-power-change-start")
    {
      uncertainty_broken_power_change_start=true;
    }    
    ...
    */
	...
	std::cerr << "NAME\n"
		  ...
		  << "\t\tNormalize the image by the light curve in file specified.  If not set, no light curve is set.  Default: none.\n"
	          << "\t-ulc, --uncertainty-loose-change\n"
		  << "\t\tAdds a loose change model for the uncertainty.  Overridden by -ubpc\n"
	          << "\t-ubpc, --uncertainty-broken-power-change\n"
		  << "\t\tAdds a broken power change model for the uncertainty.  Overrides -ulc.\n"
  ...
  // Set uncertainty model. By default, there is no change.
  Themis::uncertainty_visibility uncertainty_default;
  Themis::uncertainty_visibility_loose_change uncertainty_lc;
  Themis::uncertainty_visibility_broken_power_change uncertainty_bpc;
  Themis::uncertainty_visibility* unc_ptr;
  if (uncertainty_broken_power_change)
    unc_ptr = &uncertainty_bpc;
  else if (uncertainty_loose_change)
    unc_ptr = &uncertainty_lc;
  else
    unc_ptr = &uncertainty_default;
  Themis::uncertainty_visibility& uncertainty=(*unc_ptr);
  ...
  if (world_rank==0)
  {
    ...
    std::cerr << "image_ptr size: " << image_ptr->size()
	      ...
	      << '\n';
    for (size_t k=0; k<image.size()+uncertainty.size(); ++k)
  ...
  if ( start_parameter_list.size()>0 )
  {
    ...
    if (uncertainty_broken_power_change && uncertainty_broken_power_change_start)
    {
      if (world_rank==0)
	std::cerr << "Adding BPC uncertainty model to start values k=" << k  << "  kspl=" << kspl << std::endl;
      for (size_t j=0; j<uncertainty_bpc.size(); ++j)
	means[k++] = start_parameter_list[kspl++];
    }
    else if (uncertainty_loose_change && uncertainty_loose_change_start)
    {
      if (world_rank==0)
	std::cerr << "Adding LC uncertainty model to start values k=" << k  << "  kspl=" << kspl << std::endl;
      for (size_t j=0; j<uncertainty_lc.size(); ++j)
	means[k++] = start_parameter_list[kspl++];
    }
  }
