/*!
    \file multigauss_complex_single_deo_densestan.cpp
    \author Paul Tiede, Avery E Broderick
    \date June, 2019
...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
    ...
    else if (opt == "--max-depth")
    {
      if ( k < argc )
        max_depth = atoi(argv[k++]);
      else
      {
        if (world_rank == 0)
          std::cerr << "ERROR: An int argument must be provided after --max-depth.\n";
        std::exit(1);
      }
    }
    ...
    else if (opt == "--final-adaptation-steps" || opt == "-fas")
    {
      if ( k < argc )
        term_buffer = atoi(argv[k++]);
      else
      {
        if ( world_rank == 0 )
          std::cerr << "ERROR: An int argument must be provided after -fas.\n";
        std::exit(1);
      }
    }
    ...
    else if (opt=="-h" || opt=="--help")
    {
      if (world_rank==0)
	std::cerr << "NAME\n"
		  << "\tDriver executable for multigauss fit using deo with NUTS and a dense metric\n\n"
		  << "SYNOPSIS"
		  << "\tmpirun -np 40 multigauss_complex_single_deo_densestan -v v_file [OPTIONS]\n\n"
		  << "REQUIRED OPTIONS\n"
  ...
  Themis::sampler_deo_tempering_MCMC<Themis::sampler_stan_adapt_dense_e_nuts_MCMC> DEO(seed, L_beta, var_names, means.size());

  if (restart_flag)
    DEO.read_checkpoint("MCMC.ckpt");

  //Get the exploration sampler and set some options
  Themis::sampler_stan_adapt_dense_e_nuts_MCMC* sexplore = DEO.get_sampler();
  sexplore->set_adaptation_parameters(number_of_adaptation, save_adapt);
  sexplore->set_window_parameters(init_buffer, term_buffer, window);
  sexplore->set_max_depth(max_depth); 
  // Set a checkpoint
  ...
  std::string anneal_name = "annealing_densestan.dat";
  DEO.set_annealing_output(anneal_name);

  std::stringstream chain_file_name, state_file_name, sampler_file_name;
  chain_file_name << "chain_deo_densestan.dat";
  state_file_name << "state_deo_densestan.dat";
  sampler_file_name << "sampler_deo_dianstan.dat";
  //Set the output
  ...
  for ( size_t rep = 0; rep < number_of_rounds+1; ++rep)
  {
    if ( world_rank == 0 )
