...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
	...
	std::cerr << "NAME\n"
		  ...
		  << "SYNOPSIS"
		  << "\tmpirun -np 40 chjp_sX -vm vm_file_list -cp cp_file_list [OPTIONS]\n\n"
		  << "REQUIRED OPTIONS\n"
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    image.add_model_image(smoothed_crescent);
    //image.add_model_image(asg1);
    //image.add_model_image(sg1);
    ...
    ranges[12] = ranges[13] = 1e-6*uas2rad;
