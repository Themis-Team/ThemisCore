...
int main(int argc, char* argv[])
{
  ...
  std::ofstream sumout(sumoutname.str().c_str());
  ...
  ////////////////////
  // Do one-time object generation for things that don't evolve with the different models
  ...
  // Create a sampler object (avoid MPI communicator errors by not repeating this!)
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    Themis::model_image_score image(image_file_names[index],README_file_name);
