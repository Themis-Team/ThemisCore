/*!
    \file score_many_vacp_gain_iter.cpp
...
int main(int argc, char* argv[])
{
  ...
  bool include_theory_errors=true;
  for (int k=1; k<argc;)
  {
  ...
  std::stringstream sumoutname_all;
  sumoutname_all << "fit_summaries_all_" << std::setfill('0') << std::setw(5) << istart << "_" << std::setfill('0') << std::setw(5) << iend << ".txt";
  std::stringstream sumoutname;
  sumoutname << "fit_summaries_" << std::setfill('0') << std::setw(5) << istart << "_" << std::setfill('0') << std::setw(5) << iend << ".txt";
  std::ofstream sumout_all, sumout;
  if (world_rank==0)
  {
   sumout_all.open(sumoutname_all.str().c_str());
   sumout_all << std::setw(10) << "# Index"
	      << std::setw(15) << "I (Jy)"
	      << std::setw(15) << "M/D (uas)"
	      << std::setw(15) << "PA (deg)"
  ...
  // Read in data
  Themis::data_visibility_amplitude VM_obs(Themis::utils::global_path("eht_data/ER5/M87_VM3598.d"),"HH");
  Themis::data_closure_phase CP_obs(Themis::utils::global_path("eht_data/ER5/M87_CP3598.d"));
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    //////////////////
    // Rerun with better parameters for a number of times
    //
    for (size_t iteration=0; iteration<10; ++iteration)
    {
      ...
      // Reset the errors
      theory_error.generate_error_estimates(image_file_names,README_file_name,pmax);
      VM = theory_error.data_visibility_amplitude(VM_obs);
      CP = theory_error.data_closure_phase(CP_obs);

      // Reset the initial starting points
      means = pmax;
      ranges[0] = 0.1;
      ranges[1] = 0.1;
      ranges[2] = 0.1;
