...
int main(int argc, char* argv[])
{
  ...
  std::string visibility_amplitude_data_file=Themis::utils::global_path("eht_data/M87_2018/VM_hops_3644_M87_b3_wosb.netcal_10s_scanavg_f1_tygtd.dat");
  std::string closure_phase_data_file=Themis::utils::global_path("eht_data/M87_2018/CP_hops_3644_M87_b3_wosb.netcal_10s_scanavg_f1_tygtd.dat");
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    P.push_back(new Themis::prior_linear(-1.5*M_PI,1.5*M_PI)); // position angle
    ...
    MCMC_obj.set_tempering_schedule(200.,1.,2.0);

    // Set a checkpoint
    MCMC_obj.set_checkpoint(Ckpt_frequency,"MCMC.ckpt");
  
    // Run the Sampler
    std::stringstream Chain_A_name, Lklhd_A_name, Chi2_A_name;
    Chain_A_name << "Chain_A_" << std::setfill('0') << std::setw(5) << index << ".dat";
    Lklhd_A_name << "Lklhd_A_" << std::setfill('0') << std::setw(5) << index << ".dat";
    Chi2_A_name << "Chi2_A_" << std::setfill('0') << std::setw(5) << index << ".dat";
    
    MCMC_obj.run_sampler(L_obj, Number_of_steps, Temperature_stride, Chi2_stride, Chain_A_name.str(), Lklhd_A_name.str(), Chi2_A_name.str(), means, ranges, var_names, restart_flag, out_precision, verbosity);

    // Get the best fit and produce residual/gain files
    std::vector<double> pmax = MCMC_obj.find_best_fit(Chain_A_name.str(),Lklhd_A_name.str());
    L_obj(pmax);
    ...
    //double chi2_VM = L[0]->chi_squared(pmax);
    //double chi2_CP = L[0]->chi_squared(pmax);
