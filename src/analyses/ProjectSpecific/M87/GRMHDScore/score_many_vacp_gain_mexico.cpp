...
int main(int argc, char* argv[])
{
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    // Get the best fit and produce residual/gain files
    pmax = MCMC_obj.find_best_fit(Chain_name.str(),Lklhd_name.str());
    L_obj(pmax);

    Ndof_VM = int(VM.size()) - int(L_ogva.number_of_independent_gains()) - int(image.size());
    Ndof_CP = int(CP.size()) - int(image.size());
    Ndof = int(VM.size()+CP.size()) - int(L_ogva.number_of_independent_gains()) - int(image.size());
