...
///==========fix M/D
//    P.push_back(new Themis::prior_linear(3.619999,3.620001)); // (M/D) in uas
//    means.push_back(3.62);
//    ranges.push_back(0.00001);

///==========put PA range: Walker PA~288 288+20-270=38   288-20-270=-2
//    P.push_back(new Themis::prior_linear(-2./180.*M_PI,38./180.*M_PI)); // position angle
//    means.push_back(PAguess);
//    ranges.push_back(5./180.*M_PI);
        
          
    P.push_back(new Themis::prior_linear(-0.5*M_PI,2.5*M_PI)); // position angle
    means.push_back(PAguess);
    ...
    //VA without gain corrections
    Themis::likelihood_visibility_amplitude L_ogva(VM,image);
    L.push_back(&L_ogva);
   ...
   //======remove gain related functions: L_ogva.output_gain_corrections(gc_name.str());
    ...
    //======remove gain related functions: int Ndof_VM = int(VM.size()) - int(L_ogva.number_of_independent_gains()) - int(image.size());
    int Ndof_VM = int(VM.size()) - 0 - int(image.size());
    int Ndof_CP = int(CP.size()) - int(image.size());
    //======remove gain related functions: int Ndof = int(VM.size()+CP.size()) - int(L_ogva.number_of_independent_gains()) - int(image.size());
    int Ndof = int(VM.size()+CP.size()) - 0 - int(image.size());
