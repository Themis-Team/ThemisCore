...
int main(int argc, char* argv[])
{
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    PAguess = 108*M_PI/180.;
    //P.push_back(new Themis::prior_linear(PAguess-360*M_PI/180.,PAguess+360*M_PI/180.)); // position angle
    //means.push_back(PAguess);
    //ranges.push_back(90*M_PI/180.);
    P.push_back(new Themis::prior_linear(PAguess-5*M_PI/180.,PAguess+5*M_PI/180.)); // position angle
    means.push_back(PAguess);
    ranges.push_back(2.5*M_PI/180.);
