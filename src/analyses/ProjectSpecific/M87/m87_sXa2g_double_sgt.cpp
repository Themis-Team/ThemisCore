/*!
    \file m87_sXa2g_double_sgt.cpp
    \author Avery E Broderick
    \date Dec, 2018
    \brief Driver file for M87 analysis with smoothed xsringauss model plus components based on eponymous Challenge J+ file.  Analyzes files two at a time (i.e., four at a time).
...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
	...
	std::cerr << "NAME\n"
		  ...
		  << "\t-m, --model <string>\n"
		  << "\t\tSets the model definition string.  This may any combination of sX, a, g,\n"
  ...
  if (param_file!="")
  {
    ...
    // Get first index
    pfin >> dtmp;
    for (size_t j=0; j<start_parameter_list.size() && !pfin.eof(); j+=2)
    {
  ...
  // Parse model_glob
  bool add_sX=false, add_a=false, add_g=false;
  size_t number_a=0;
  size_t mgi=0;
  ...
  for (size_t index=size_t(istart); index<size_t(iend); index+=2)
  {
    // Read in data
    ...
    CP.add_data(cp_file_names[index+1]);
    
    if (world_rank==0)
    {
      std::cout << "Using data files:\n"
		<< '\t' << vm_file_names[index] << '\n'
    ...
    Themis::model_image_symmetric_gaussian model_g;
	...
	std::cout << "Pushed back sX prior list, now at " << means.size() << std::endl;
      ...
      //   0 Itot
      P.push_back(new Themis::prior_linear(0.0,1.8));
      means.push_back(0.6);
	...
	//   0 Itot 
	P.push_back(new Themis::prior_linear(0,1.8));
	means.push_back(0.1);
	ranges.push_back(0.1);
	//   1 Size
	P.push_back(new Themis::prior_linear(0.0,200*uas2rad));
    ...
    if (world_rank==0)
    {
      ...
      std::ofstream pout("prior_ranges.txt");
    ...
    {
      std::cout << "Finished pusing prior lists, now at " << means.size() << std::endl;
      std::ofstream pout("prior_ranges_2.txt");
      for (size_t j=0; j<means.size(); ++j)
	pout << std::setw(5) << j
	     << std::setw(15) << means[j]
	     << std::setw(15) << ranges[j]
    ...
    std::stringstream VA1_res_name, VA2_res_name, CP_res_name, gc1_name, gc2_name;
    VA1_res_name << "VA_residuals_hi_" << std::setfill('0') << std::setw(5) << index << ".d";
    VA2_res_name << "VA_residuals_lo_" << std::setfill('0') << std::setw(5) << index << ".d";
    CP_res_name << "CP_residuals_" << std::setfill('0') << std::setw(5) << index << ".d";
    gc1_name << "gain_corrections_hi_" << std::setfill('0') << std::setw(5) << index << ".d";
    gc2_name << "gain_corrections_lo_" << std::setfill('0') << std::setw(5) << index << ".d";
    L_obj(means);
    ...
    L[2]->output_model_data_comparison(CP_res_name.str());
  
    // Create a sampler object
    Themis::sampler_differential_evolution_tempered_MCMC MCMC_obj(seed+world_rank);
  
    // Generate a chain
    int Number_of_chains = 240; //120;
    int Number_of_temperatures = 32; //16; //40; //8;
    ...
    int Ndof_VM = int(VM1.size()) - int(lva1.number_of_independent_gains()) + int(VM2.size()) - int(lva2.number_of_independent_gains()) - int(image.size());
    int Ndof_CP = int(CP.size()) - int(image.size());
    int Ndof = int(VM1.size()+VM2.size()+CP.size()) - int(lva1.number_of_independent_gains()) - int(lva2.number_of_independent_gains()) - int(image.size());

    double chi2_VM = L[0]->chi_squared(pmax) + L[1]->chi_squared(pmax);
    double chi2_CP = L[2]->chi_squared(pmax);
    ...
    chi2_VM = L[0]->chi_squared(pmax) + L[1]->chi_squared(pmax);
    chi2_CP = L[2]->chi_squared(pmax);
