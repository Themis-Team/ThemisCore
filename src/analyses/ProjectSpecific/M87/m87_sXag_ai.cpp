/*!
    \file m87_sXag_ai.cpp
    \author Avery E Broderick
    \date Dec, 2018
    \brief Driver file for M87 analysis with smoothed xsringauss model plus components based on eponymous Challenge J+ file.
...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
	...
	std::cerr << "NAME\n"
		  ...
		  << "\t\tin that order, with only one of each (so sXag, sX, sXg, ag, etc.).\n"
		  << "\t--seed <int>\n"
  ...
  for (size_t index=size_t(istart); index<size_t(iend); ++index)
  {
    ...
    Themis::sampler_affine_invariant_tempered_MCMC MCMC_obj(seed+world_rank);
