/*!
    \file J1924_mexico_hilo.cpp
    \author Avery E Broderick
    \date June, 2019
    \brief Driver file for J1924 analysis of HI+LO with geometric models:
    ...
    \details Takes file lists generated via something like:
	     ...
	     These must be passed as -vm <file> and -cp <file> options.  Expects HI and LO
	     band files are interleaved on subsequent lines and fits both simultaneously.
    \todo 
*/
...
int main(int argc, char* argv[])
{
  ...
  for (size_t index=size_t(istart); index<size_t(iend); index+=2)  // Jump by two 
  {
    ...
    int Ndof_VM = int(VM1.size()) - int(lva1.number_of_independent_gains()) + int(VM2.size()) - int(lva2.number_of_independent_gains()) - int(image.size()) + 2;
    int Ndof_CP = int(CP.size()) - int(image.size()) + 2;
    int Ndof = int(VM1.size()+VM2.size()+CP.size()) - int(lva1.number_of_independent_gains()) - int(lva2.number_of_independent_gains()) - int(image.size()) + 2;
