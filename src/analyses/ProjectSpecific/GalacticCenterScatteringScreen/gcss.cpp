...
int main(int argc, char* argv[])
{
  ...
  // 1. Read in data
  Themis::data_visibility V1, V2, V3;
  V1.set_default_frequency(86e9);
  V2.set_default_frequency(86e9);
  V3.set_default_frequency(86e9);
  V1.add_data(Themis::utils::global_path("gmva_data/SgrA/V_hops_sgra_MB007_selfcal_scanavg_f3_t10mJy_tygtd.dat"),"HH");
  V2.add_data(Themis::utils::global_path("gmva_data/SgrA/V_hops_sgra_MJ001A_selfcal_scanavg_f3_t10mJy_tygtd.dat"),"HH");
  V3.add_data(Themis::utils::global_path("gmva_data/SgrA/V_hops_sgra_MJ001B_selfcal_scanavg_f3_t10mJy_tygtd.dat"),"HH");
  ...
  {
    lv.push_back( new Themis::likelihood_visibility(V1,model) );
    lv.push_back( new Themis::likelihood_visibility(V2,model) );
    lv.push_back( new Themis::likelihood_visibility(V3,model) );
  ...
  for (size_t rep=round_start; rep<=Number_of_reps; ++rep)
  {
    ...
    for (size_t j=0; j<L.size(); ++j)
    {
      ...
      if (world_rank==0)
      {
	std::cout << "V" << j << " rchi2: " << V_rchi2  << " / " << 2*Nd[j] << " - " << Nparam << " - " << Ngains_list[j] << std::endl;
	// Summary file values
	sumout << std::setw(15) << V_rchi2 / ( 2*Nd[j] - Nparam - Ngains_list[j]);
      }
    }
