...
int main(int argc, char* argv[])
{
  ...
  Themis::model_image_smooth ssmring(smring);
  ...
  means.push_back(0.8);
  //   1 Outer size R
  P.push_back(new Themis::prior_linear(5.0*uas2rad,85.0*uas2rad/2.0));
  ...
  //stretch
  P.push_back(new Themis::prior_linear(0.0, 0.50));
  means.push_back(1e-2);
  //stretch PA
  P.push_back(new Themis::prior_linear(-M_PI/2, M_PI/2));
  means.push_back(0.0);
  //11 smoothing size
  P.push_back(new Themis::prior_linear(1.0*uas2rad,20.0*uas2rad));
  ...
  means.push_back(0.2);
  //17 Gaussian size 
  P.push_back(new Themis::prior_linear(20.0*uas2rad, 100.0*uas2rad));
  ...
  for ( size_t rep = round_start; rep < number_of_rounds; ++rep)
  {
    ...
    if (world_rank==0)
    {
      ...
      sumout << std::setw(15) << "stretch"
             << std::setw(15) << "stretch_pa"
             << std::setw(15) << "sg_size"
             << std::setw(15) << "sg_asym"
             << std::setw(15) << "sg_xi"
             << std::setw(15) << "x0"
