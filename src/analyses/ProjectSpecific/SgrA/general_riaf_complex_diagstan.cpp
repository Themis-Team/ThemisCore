...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
    ...
    else if (opt=="-h" || opt=="--help")
    {
      if (world_rank==0)
	std::cerr << "NAME\n"
		  << "\tDriver executable for the general riaf fit using deo with diagonal stan\n\n"
		  << "SYNOPSIS"
		  << "\tmpirun -np 40 multigauss_complex_single_deo_diagstan -v v_file [OPTIONS]\n\n"
		  << "REQUIRED OPTIONS\n"
		  ...
		  << "\t\tDefaults to 100.\n"
                  << "\t--max-depth <int>\n"
                  << "\t\tSets the max tree depth for NUTS. Default is 10.\n"
  ...
  Themis::sampler_stan_adapt_diag_e_nuts_MCMC* skernel = DEO.get_sampler();
  skernel->set_adaptation_parameters(number_of_adaptation, save_adapt);
  skernel->set_window_parameters(init_buffer, term_buffer, window);
  skernel->set_max_depth(max_depth); 
  // Set a checkpoint
  ...
  std::string anneal_name = "annealing_diagstan.dat";
  DEO.set_annealing_output(anneal_name);

  std::stringstream chain_file_name, state_file_name, sampler_file_name;
  chain_file_name << "chain_deo_diagstan.dat";
  state_file_name << "state_deo_diagstan.dat";
  sampler_file_name << "sampler_deo_diagstan.dat";
  //Set the output
  int out_precision = 8;
