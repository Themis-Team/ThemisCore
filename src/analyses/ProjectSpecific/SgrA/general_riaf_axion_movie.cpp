/*!
    \file general_riaf_axion_movie.cpp
    \author Zhiren Wang, Avery Broderick
    \date May, 2020
    \brief Driver file for generating movie frames for a RIAF with an axion cloud.
    \details TBD
*/
...
int main(int argc, char* argv[])
{
  ...
  for ( size_t p = size_t(start_line()); p < std::min(end_line_index,start_parameter_list.size()); ++p )
  {
    ...
    */
    ...
    {
      // std::cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<" << std::endl;
      riaf.set_tobs((j-frames_per_period())*axion_period/double(frames_per_period()));
      riaf.generate_model(params);
