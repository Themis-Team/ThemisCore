...
int main(int argc, char* argv[])
{
  ...
  for (int k=1; k<argc;)
  {
    ...
    {
      if (k<argc)
	mring_order = atoi(argv[k++]);
      else
      {
	if (world_rank==0)
	  std::cerr << "ERROR: A string argument must be provided after --morder.\n";
	std::exit(1);
      }
  ...
  //Themis::model_image_stretch smring(mring);
  Themis::model_image_smooth ssmring(mring);
  Themis::model_image_symmetric_gaussian gauss;
  model_components.push_back(&ssmring);
  model_components.push_back(&gauss);
  ...
  station_gain_priors[5] = 0.1;//PV
  station_gain_priors[6] = 0.01;//SM
  station_gain_priors[7] = 0.5;//SP
  */
  /*
  station_gain_priors[0] = 0.2;//AA
  station_gain_priors[1] = 0.2;//AP
  station_gain_priors[2] = 0.2;//AZ
  station_gain_priors[3] = 0.2;//JC
  station_gain_priors[4] = 0.2;//LM
  station_gain_priors[5] = 0.2;//PV
  station_gain_priors[6] = 0.2;//SM
  station_gain_priors[7] = 0.2;//SP
  */
  ...
  //   0 Itot
  P.push_back(new Themis::prior_linear(0.05,4.));
  means.push_back(0.8);
  //   1 Outer size R
  P.push_back(new Themis::prior_linear(10.0*uas2rad,85.0*uas2rad/2));
  means.push_back(1.31508e-10);
  //Mring expansion
  for (int i = 0; i < mring_order; ++i){
      //Real
      P.push_back(new Themis::prior_linear(0.0, 0.5));
      means.push_back(0.1);
      //Imag
  ...
  means.push_back(7.67314e-11);
  //12 smoothing asym
  P.push_back(new Themis::prior_linear(0.00, 1e-6));
  means.push_back(1e-7);
  //13 smoothing pos ang
  ...
  P.push_back(new Themis::prior_linear(-1e-6*uas2rad,1e-6*uas2rad));
  means.push_back(0.0);
  //16 Gaussian floor flux
  P.push_back(new Themis::prior_linear(0.05, 4.));
  means.push_back(0.2);
  //17 Gaussian size 
  P.push_back(new Themis::prior_linear(20.0*uas2rad, 200.0*uas2rad));
  means.push_back(50.0*uas2rad);
  //18. Pin the Gaussian to the center
  P.push_back(new Themis::prior_linear(-1e-6*uas2rad,1e-6*uas2rad));
  means.push_back(0.0);
  P.push_back(new Themis::prior_linear(-1e-6*uas2rad,1e-6*uas2rad));
  means.push_back(0.0);
  ...
  for (size_t mc=0; mc<model_list.size(); ++mc)
  {
    ...
    {
      //   0 Itot
        std::cerr << "Large gauss priors\n";
      P.push_back(new Themis::prior_linear(0.0,4.0));
      means.push_back(1.0);
      //   1 Size
      P.push_back(new Themis::prior_logarithmic(1e3*uas2rad,1e9*uas2rad));
      means.push_back(9.5e3*uas2rad);
    }
    else
    {
      //   0 Itot
      ...
      means.push_back(2.5);
      //   1 Size
      P.push_back(new Themis::prior_linear(5*uas2rad,1e2*uas2rad));
    ...
    if (model_list[mc]=="G" || model_list[mc]=="A" || model_list[mc]=="g") 
    {
  ...
  if (model_noise) { 
	...
	P.push_back(new Themis::prior_linear(std::log(noise_lo[nt]),std::log(noise_hi[nt])));
      ...
      if (Number_start_params <= (int)image.size())
      {
      	means.push_back(std::log(0.004)); // noise threshold
    ...
    /**************** END oF imrecad_complex_logconfig-opt-noise.cpp bits ****************/
  ...
  // if ((model_noise) && (Number_start_params < Number_of_pixels_x*Number_of_pixels_y+5)) { // RG: reset the noise parameters -p only covers themage parameters, still assume we only themage (without nuisance Gaussians, rings etc)...
    ...
    // if (model_noise_priors_sgra) {
      ...
      // // std::cout<<"P.size()="<<P.size()<<",means.size()="<<means.size()<<std::endl;
  ...
  // DEBUG
  std::cerr << world_rank << " AFTER restart_flag. " << round_start << "\n";
  ...
  for ( size_t rep = round_start; rep < number_of_rounds; ++rep)
  {
    ...
    if (world_rank==0)
    {
      ...
      sumout << std::setw(15) << "I"
             << std::setw(15) << "Rout";
      for ( int m = 0; m < mring_order; ++m )
          sumout << std::setw(15) << "mre"+std::to_string(m)
                 << std::setw(15) << "mim"+std::to_string(m);
      sumout << std::setw(15) << "sg_size"
             << std::setw(15) << "sg_asym"
             << std::setw(15) << "sg_xi"
             << std::setw(15) << "x0"
             << std::setw(15) << "y0"
