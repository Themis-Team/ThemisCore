/*!
  \file gaussian_image_comparison_numerical.cpp
  \author
  \date Apr 2017
  \test Gaussian model with numerically calculated visibility amplitude data
  \brief Fits Gaussian model to visibility amplitude data

  \details Uses numerical visibility amplitude and hence tests the FFT
  routine as well at the expense of some computational performance
  over the analytic version in gaussian_image_comparison.cpp . Scattering is applied.
*/
...
int main(int argc, char* argv[])
{
  ...
  ranges.push_back(0.05*image_scale);
  ranges.push_back(0.05*image_scale);
  ranges.push_back(0.05*M_PI);
  ...
  MC_obj.run_sampler(L_obj, Number_of_steps, 
                      Temperature_stride, Chi2_stride, 
