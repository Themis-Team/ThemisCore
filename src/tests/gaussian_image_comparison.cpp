/*!
  \file tests/gaussian_image_comparison.cpp
  \authors Avery Broderick, Roman Gold, Mansour Karami, Jorge A. Preciado 
  \date Apr 2017
  \test Gaussian model with visibility amplitude data
  \brief Fits a Gaussian model to visibility amplitude data.

  \details Uses analytical visibilities (fast). See
  gaussian_image_comparison_numerical.cpp for the slower version that
  performs the FFTs numerically and compare the result. Scattering is
  applied.
  
  \image html Gaussian-Triangle.png “Triangle Plot”
*/
...
int main(int argc, char* argv[])
{
  ...
  var_names.push_back("$\\sigma_{u}$");
  var_names.push_back("$\\sigma_{v}$");
